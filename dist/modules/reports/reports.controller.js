...
function utamaOnePercentFee(m) {
    ...
    const feeRow = t.includes('biaya percepatan pencairan qris') || (t.includes('biaya percepatan') && t.includes('qris')) || t.includes('biaya percepatan pencairan');
...
async function showReports(req, res) {
    try {
        ...
        // Transaksi ditautkan ke site lewat qrisAccountId -> akun -> site (JSON account-sites.json).
        // Fase 6: alias-tenant -> paksa lihat site-nya saja (abaikan ?site= dari query).
        const _aliasScope = alias_access_service_1.getSiteScopeForUser(req.session.user);
        const sites = _aliasScope ? site_service_1.listSites().filter((s) => s.id === _aliasScope) : site_service_1.listSites();
        const accountsAll = await database_1.db.qrisAccount.findMany({ select: { id: true } });
        const accSiteMap = site_service_1.getAccountSiteMap();
        const noneAccountIds = accountsAll.filter((a) => !accSiteMap[a.id]).map((a) => a.id);
        const hasNoneBucket = noneAccountIds.length > 0;
        // Bucket rincian: hanya site yang punya >=1 akun, plus "Tanpa site" bila ada.
        const siteBuckets = sites
            .map((s) => ({ key: s.id, name: s.name, accountIds: accountsAll.filter((a) => accSiteMap[a.id] === s.id).map((a) => a.id) }))
        ...
        // Filter site terpilih: '' = Semua Site (gabungan), siteId, atau 'none'.
        // Alias-tenant: dipaksa ke site-nya (tak bisa lihat site lain).
        ...
        // Bungkus where dgn batas akun bila site tertentu dipilih (Semua Site = tanpa batas).
        ...
        // FEE (QRIS Biaya Layanan) + FEE2 (Utama 1%) dari MUTASI (transactionTime dalam periode), dijumlah per akun.
        const [_qrisMuts, _utamaMuts] = await Promise.all([
            database_1.db.mutation.findMany({ where: { walletCategory: 'qris', transactionTime: { gte: from, lte: to } }, select: { amount: true, rawDataJson: true, qrisAccountId: true } }),
            database_1.db.mutation.findMany({ where: { walletCategory: 'utama', transactionTime: { gte: from, lte: to } }, select: { amount: true, rawDataJson: true, qrisAccountId: true } }),
        ]);
        const feeQrisByAcc = {};
        ...
        const [totalAll, paidAll, expiredAll, openAll, depSuccessAll, depFailedAll, manualReviewAll, paidAggAll] = await Promise.all([
            ...
            // FIX #8: hitung 'open' LANGSUNG (bukan total-paid-expired) agar akurat & konsisten dgn API.
            database_1.db.transaction.count({ where: { ...overallWhere, statusPay: 'open' } }),
            // FIX #7: total deposit keseluruhan utk footer tabel.
        ...
        // Grafik (nominal terbayar by paidAt, per HARI WIB).
        // FIX #4: SELALU cakup seluruh rentang. Jika > 31 hari, dikelompokkan jadi <=31 batang
        //         (N hari/batang) supaya tak ada data yang hilang diam-diam.
        // FIX #5: semua kueri batang dijalankan PARALEL (bukan await berurutan) -> jauh lebih cepat.
        ...
        const bucketResults = await Promise.all(buckets.map(async (bk) => {
            const [cnt, agg] = await Promise.all([
                ...
                database_1.db.transaction.aggregate({ where: withAccount({ statusPay: 'paid', paidAt: { gte: bk.start, lte: bk.end } }), _sum: { finalAmount: true } }),
            ]);
