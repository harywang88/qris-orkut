...
function getScopeAccountIds(user) {
    const scope = alias_access_service_1.getSiteScopeForUser(user);
    if (!scope)
        return null;
    return site_service_1.accountIdsForSite(scope);
}
...
function buildMaderaPresentedMutations(account, topupHistory, settlements, currentBalance) {
    ...
    const __maderaSiteName = site_service_1.siteNameForAccount(account.id);
...
async function showDashboard(req, res) {
    try {
        ...
        const _resolveSite = site_service_1.buildResolver();
...
async function showTransactions(req, res, opts = {}) {
    ...
    try {
        ...
        if (site === 'none') {
            ...
            const _map = site_service_1.getAccountSiteMap();
            siteAccountIds = _allAccts.filter((a) => !_map[a.id]).map((a) => a.id);
        }
        else if (site) {
            siteAccountIds = site_service_1.accountIdsForSite(site);
        }
        ...
        const _scopeSite = alias_access_service_1.getSiteScopeForUser(req.session.user);
        const sites = _scopeSite ? site_service_1.listSites().filter((s) => s.id === _scopeSite) : site_service_1.listSites();
    ...
    try {
        ...
        (async () => {
            try {
                ...
                else { logger_1.logger.warn({ id, msg: urlRes && urlRes.message }, 'captureProof: url gagal'); }
            }
            catch (err) { logger_1.logger.error({ err, id }, 'captureProof background error'); }
...
async function buildMaderaRealRowsForAccount(account, source, limit) {
    ...
    const __accSiteName = site_service_1.siteNameForAccount(account.id);
    return rows.map((m) => {
...
async function getMutationsJson(req, res) {
    try {
        ...
        if (source === 'qris') {
            ...
            const resolveSite = site_service_1.buildResolver();
            ...
            if (bucket === 'madera' && targetAccount && presentedMutations.length === 0) {
                ...
                if (inferred.mutations.length > 0) {
                    ...
                    const __infSiteName = site_service_1.siteNameForAccount(targetAccount.id);
...
async function computeMaderaReconcileForAccount(account) {
    ...
    const mapped = items.map((it) => madera_history_service_1.mapMaderaFeedItem(account.id, it)).filter(Boolean);
...
async function showAkunAlias(req, res) {
    try {
        const accounts = alias_access_service_1.listAccounts(req.session.user);
        res.render('settings/akun-alias', {
            title: 'Akun Alias',
            menuDefs: alias_access_service_1.MENU_DEFS,
            accounts,
            sites: site_service_1.listSites(),
        });
    }
    catch (err) {
...
async function getAliasAccountsApi(req, res) {
    try {
        res.json({ ok: true, accounts: alias_access_service_1.listAccounts(req.session.user), menuDefs: alias_access_service_1.MENU_DEFS });
    }
    catch (err) {
...
async function createAliasApi(req, res) {
    try {
        const b = req.body || {};
        await alias_access_service_1.createAlias({ username: b.username, name: b.name, password: b.password, perms: b.perms || {}, siteScope: b.siteScope });
        res.json({ ok: true });
    }
    catch (err) {
...
async function updateAliasApi(req, res) {
    try {
        ...
        const b = req.body || {};
        await alias_access_service_1.updateAlias(username, { name: b.name, password: b.password, perms: b.perms, siteScope: b.siteScope });
        res.json({ ok: true });
    }
    catch (err) {
...
async function deleteAliasApi(req, res) {
    try {
        ...
        alias_access_service_1.deleteAlias(username);
        res.json({ ok: true });
    }
    catch (err) {
...
async function showSettlement(req, res) {
    try {
        ...
        const _siteResolve = site_service_1.buildResolver();
...
async function showGenerateQr(req, res) {
    try {
        ...
        let accounts = site_service_1.attachSiteInfo(accountsRaw);
        let sites = site_service_1.listSites();
        // Fase 6: alias-tenant -> hanya site & akun miliknya.
        const _scope = alias_access_service_1.getSiteScopeForUser(req.session.user);
        if (_scope) {
