...
exports.getTransactionsSnapshotApi = getTransactionsSnapshotApi;
exports.getLatestTransactionsApi = getLatestTransactionsApi;
exports.showMutations = showMutations;
exports.showMutationsQris = showMutationsQris;
exports.streamMutationsSse = streamMutationsSse;
exports.showMutationsUtama = showMutationsUtama;
exports.showMutationsMadera = showMutationsMadera;
exports.showMaderaNobuHistory = showMaderaNobuHistory;
exports.handleCaptureSettlementProofApi = handleCaptureSettlementProofApi;
exports.getSettlementProofApi = getSettlementProofApi;
exports.getSettlementProofsListApi = getSettlementProofsListApi;
exports.getMaderaNobuWebviewApi = getMaderaNobuWebviewApi;
exports.getMaderaNobuHistoryApi = getMaderaNobuHistoryApi;
exports.getBridgeStatusApi = getBridgeStatusApi;
exports.getMutationsJson = getMutationsJson;
exports.getQrisReconcileApi = getQrisReconcileApi;
exports.getGenerateQrStatusApi = getGenerateQrStatusApi;
exports.showAkunAlias = showAkunAlias;
exports.getAliasAccountsApi = getAliasAccountsApi;
exports.createAliasApi = createAliasApi;
exports.updateAliasApi = updateAliasApi;
exports.deleteAliasApi = deleteAliasApi;
exports.showSettlement = showSettlement;
exports.handleSettlementBankInquiryApi = handleSettlementBankInquiryApi;
exports.handleAccountTransferApi = handleAccountTransferApi;
exports.handleRetryAutoPinApi = handleRetryAutoPinApi;
exports.handleBankListApi = handleBankListApi;
exports.handleCreateSettlement = handleCreateSettlement;
exports.showLoginLogs = showLoginLogs;
exports.showAliases = showAliases;
exports.checkWebGamePanelApi = checkWebGamePanelApi;
exports.showAccountSettings = showAccountSettings;
exports.getQrisAccountsJson = getQrisAccountsJson;
exports.handleRefreshAccountBalanceApi = handleRefreshAccountBalanceApi;
exports.showGenerateQr = showGenerateQr;
exports.handleDashboardGenerateQr = handleDashboardGenerateQr;
exports.getQrisTemplate = getQrisTemplate;
...
const orkut_web_report_service_1 = require("../../shared/orkut-web-report.service");
...
const dashboard_generate_qr_service_1 = require("../../shared/dashboard-generate-qr.service");
const madera_history_service_1 = require("../../shared/madera-history.service");
...
function getScopeAccountIds(user) {
    const scope = (0, alias_access_service_1.getSiteScopeForUser)(user);
    if (!scope)
        return null;
    return (0, site_service_1.accountIdsForSite)(scope);
}
function accountInScope(user, accountId) {
...
function normalizeTransactionPeriod(value) {
    if (value === 'today' || value === 'yesterday' || value === '7d' || value === '30d' || value === 'custom') {
        return value;
    }
    return 'all';
}
function getTransactionDateRange(query) {
...
function normalizeMutationSource(value) {
    if (value === 'utama' || value === 'madera' || value === 'qris')
        return value;
    return 'qris';
}
function getMutationSourceLabel(source) {
    if (source === 'qris')
        return 'QRIS';
    if (source === 'utama')
        return 'Saldo Utama';
...
function getMutationSourceSubtitle(source) {
    if (source === 'qris') {
...
function getMutationApiUrl(source) {
    return (0, base_path_1.withBasePath)(`/dashboard/api/mutations?source=${source}&limit=500`, config_1.config.APP_BASE_PATH);
}
const DETAIL_ENRICH_COOLDOWN_MS = 5 * 60 * 1000;
const qrisDetailEnrichAttempts = new Map();
function normalizeQrisBucket(value) {
...
function normalizeMutationCategory(value) {
    if (value === 'qris' || value === 'utama' || value === 'madera')
        return value;
    return null;
}
function hasExplicitUtamaMarker(text) {
    return (text.includes('pencairan qris')
...
function readMutationCategory(rawDataJson, storedCategory) {
    ...
    try {
        ...
        if (description) {
            if (text.includes('pencairan saldo qris'))
                return 'qris';
            if (hasExplicitMaderaMarker(text))
                return 'madera';
            if (hasExplicitUtamaMarker(text))
                return 'utama';
        }
        if (parsedStored)
            return parsedStored;
        ...
        if (description) {
            return (0, orkut_panel_service_1.classifyOrkutMutationDescription)(description);
        }
    }
    catch {
...
function parseRawMutationPayload(rawDataJson) {
    try {
        const parsed = JSON.parse(rawDataJson);
...
function hasDisplayTimeSeconds(value) {
    return typeof value === 'string' && /\d{2}:\d{2}:\d{2}$/.test(value.trim());
}
function resolveMutationReferenceCandidate(raw, fallbackRrn) {
    const explicitReference = readString(raw.issuer_ref) ||
        readString(raw.rrn) ||
        readString(raw.ref) ||
    ...
    const descriptionReferenceSource = readString(raw.description) ||
...
function shouldAttemptQrisDetailEnrichment(accountId, rawId) {
...
function markQrisDetailEnrichmentAttempt(accountId, rawId) {
    qrisDetailEnrichAttempts.set(`${accountId}:${rawId}`, Date.now());
}
function formatMutationTimestamp(value) {
...
function inferMaderaDirection(description, fallbackType) {
    const text = description.toLowerCase();
    if (text.includes('ke madera')
        || text.includes('topup madera')
        || text.includes('pindah saldo ke madera')
        || text.includes('saldo masuk madera')) {
        return 'credit';
    }
    if (text.includes('dari madera')
...
async function inferMaderaStateFromAppHistory(account) {
    ...
    const related = history.mutations
        ...
        const description = readString(raw.keterangan) ||
            readString(raw.description) ||
            '';
        if ((0, orkut_panel_service_1.classifyOrkutMutationDescription)(description) !== 'madera') {
            return null;
        }
        return {
            id: `${account.id}:${mutation.rawHash}`,
            amount: mutation.amount,
            type: inferMaderaDirection(description, mutation.type),
    ...
    const inferredBalance = typeof account.lastMaderaBalance === 'number'
        ...
        : Math.max(0, related.reduce((sum, item) => sum + (item.type === 'credit' ? item.amount : -item.amount), 0));
...
function parseSettlementNoteMarkers(note) {
...
function isMaderaTopupDescription(description) {
    const text = description.toLowerCase();
    return text.includes('pindah saldo ke madera') || text.includes('topup madera');
}
function extractMaderaReference(description) {
...
function buildMaderaPresentedMutations(account, topupHistory, settlements, currentBalance) {
    const entries = [];
    let note = null;
    for (const topup of topupHistory) {
        const topupReference = extractMaderaReference(topup.description)
            || topup.referenceNo
            || null;
    ...
    const dedupedEntries = Array.from(new Map(entries.map((entry) => {
    ...
    const knownCurrentBalance = typeof currentBalance === 'number'
        ? currentBalance
        : account.lastMaderaBalance;
    const sumCredit = dedupedEntries.reduce((sum, entry) => (entry.type === 'credit' ? sum + entry.amount : sum), 0);
    const sumDebit = dedupedEntries.reduce((sum, entry) => (entry.type === 'debit' ? sum + entry.amount : sum), 0);
    ...
    const netMutation = chronologicalEntries.reduce((sum, entry) => (entry.type === 'credit' ? sum + entry.amount : sum - entry.amount), 0);
    ...
    const __maderaSiteName = (0, site_service_1.siteNameForAccount)(account.id);
...
function parseMutationPresentation(rawDataJson, fallback) {
    const raw = parseRawMutationPayload(rawDataJson);
    const rawTimestamp = readString(raw.tanggal) ||
        readString(raw.date) ||
        readString(raw.created_at) ||
        readString(raw.waktu);
    const rawDescription = readString(raw.keterangan) ||
        readString(raw.description) ||
        readString(raw.note) ||
        readString(raw.ket);
    const rawStatus = readString(raw.status).toUpperCase();
    const brandName = readNestedString(raw.brand, 'name') ||
        readString(raw.brand_name) ||
        readString(raw.brand);
    const issuerCandidate = readString(raw.bank) ||
    ...
    const directSender = readString(raw.buyer_ref) ||
...
async function getQrisBalanceSummary() {
    ...
    const summary = (0, orkut_panel_service_1.summarizeOrkutAccountBalances)(accounts);
    return summary.syncedAccounts > 0 ? summary : null;
}
async function renderMutationsPage(req, res, source) {
...
async function showDashboard(req, res) {
    try {
        ...
        const [totalToday, paidToday, activeAccounts, totalClients, failedDeposits, manualReview, queueLag,] = await Promise.all([
            database_1.db.transaction.count({ where: { createdAt: { gte: todayStart, lte: todayEnd } } }),
            database_1.db.transaction.count({
                where: { statusPay: 'paid', paidAt: { gte: todayStart, lte: todayEnd } },
            }),
            database_1.db.qrisAccount.count({ where: { status: 'active' } }),
            database_1.db.client.count({ where: { status: 'active' } }),
            database_1.db.transaction.count({ where: { statusBot: 'deposit_failed' } }),
            database_1.db.transaction.count({ where: { statusBot: 'manual_review' } }),
            database_1.db.transaction.count({ where: { statusPay: 'paid', statusBot: 'deposit_queued' } }),
        ]);
        ...
        const recentTx = await database_1.db.transaction.findMany({
        ...
        const recentTransactionsRaw = await database_1.db.transaction.findMany({
            take: 10,
        ...
        const _resolveSite = (0, site_service_1.buildResolver)();
        const recentTransactions = recentTransactionsRaw.map((tx) => Object.assign({}, tx, { siteName: _resolveSite(tx.qrisAccountId).siteName }));
        const qrisAccountsRaw = await database_1.db.qrisAccount.findMany({ orderBy: { code: 'asc' } });
        ...
        const paidAggAcc = await database_1.db.transaction.groupBy({
            by: ['qrisAccountId'],
        ...
        logger_1.logger.error({ err }, 'showDashboard error');
...
// ── Paid Transactions ─────────────────────────────────────────────────────────
async function showTransactions(req, res, opts = {}) {
    ...
    try {
        ...
        if (statusPay)
            where.statusPay = statusPay;
        if (statusBot)
            where.statusBot = statusBot;
        if (accountCode) {
        ...
        const scopeIds = getScopeAccountIds(req.session.user);
        let siteAccountIds = null;
        if (site === 'none') {
            const _allAccts = await database_1.db.qrisAccount.findMany({ select: { id: true } });
            const _map = (0, site_service_1.getAccountSiteMap)();
            siteAccountIds = _allAccts.filter((a) => !_map[a.id]).map((a) => a.id);
        }
        else if (site) {
            siteAccountIds = (0, site_service_1.accountIdsForSite)(site);
        }
        let effectiveAccountIds = null;
        if (scopeIds && siteAccountIds)
            effectiveAccountIds = siteAccountIds.filter((id) => scopeIds.includes(id));
        else if (scopeIds)
            effectiveAccountIds = scopeIds;
        else if (siteAccountIds)
            effectiveAccountIds = siteAccountIds;
        if (effectiveAccountIds)
            where.qrisAccountId = { in: effectiveAccountIds };
        // Menu Transaction: KUNCI hanya paid (server-side) — QR generate/open/expired mustahil masuk walau via URL.
        if (paidOnly)
            where.statusPay = 'paid';
        ...
        const [clients, qrisAccounts, paidAgg, statusPayAgg, statusBotAgg] = await Promise.all([
            database_1.db.client.findMany({ select: { id: true, name: true }, orderBy: { name: 'asc' } }),
            database_1.db.qrisAccount.findMany({ select: { id: true, code: true, merchantName: true }, orderBy: { code: 'asc' } }),
            database_1.db.transaction.aggregate({ where, _sum: { finalAmount: true, feeAmount: true } }),
            database_1.db.transaction.groupBy({
                by: ['statusPay'],
                where,
                _count: { _all: true },
            }),
        ...
        const statusPayMap = statusPayAgg.reduce((acc, row) => {
            acc[row.statusPay] = row._count._all;
            return acc;
        }, {});
        const statusBotMap = statusBotAgg.reduce((acc, row) => {
        ...
        const _scopeSite = (0, alias_access_service_1.getSiteScopeForUser)(req.session.user);
        const sites = _scopeSite ? (0, site_service_1.listSites)().filter((s) => s.id === _scopeSite) : (0, site_service_1.listSites)();
        ...
        logger_1.logger.error({ err }, 'showTransactions error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
// ── Mutations ─────────────────────────────────────────────────────────────────
async function getTransactionsSnapshotApi(req, res) {
    try {
        const qrIds = Array.isArray(req.body?.qrIds)
            ? req.body.qrIds.filter((value) => typeof value === 'string' && value.trim().length > 0).slice(0, 100)
            : [];
        if (qrIds.length === 0) {
            res.json({ ok: true, transactions: [] });
            return;
        }
        const transactions = await database_1.db.transaction.findMany({
            where: { qrId: { in: qrIds } },
            select: {
                qrId: true,
                qrisAccountId: true,
        ...
        logger_1.logger.error({ err }, 'getTransactionsSnapshotApi error');
...
async function getLatestTransactionsApi(req, res) {
    try {
        const take = Math.min(50, Math.max(1, parseInt(String(req.query?.take ?? '30'), 10) || 30));
        const _scopeIds = getScopeAccountIds(req.session.user);
        const transactionsRaw = await database_1.db.transaction.findMany({
        ...
        logger_1.logger.error({ err }, 'getLatestTransactionsApi error');
        res.status(500).json({ ok: false, error: 'Gagal mengambil transaksi terbaru.' });
    }
}
async function showMutations(req, res) {
    try {
        res.redirect((0, base_path_1.withBasePath)('/dashboard/mutations/qris', config_1.config.APP_BASE_PATH));
    }
    catch (err) {
        logger_1.logger.error({ err }, 'showMutations error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
async function showMutationsQris(req, res) {
    try {
        const accounts = await database_1.db.qrisAccount.findMany({
            where: { status: 'active' },
            orderBy: { code: 'asc' },
        ...
        res.render('mutations/qris', {
            ...
            sites,
            streamUrl: (0, base_path_1.withBasePath)('/dashboard/api/mutations/stream', config_1.config.APP_BASE_PATH),
        });
    }
    catch (err) {
        logger_1.logger.error({ err }, 'showMutationsQris error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
function formatOutboxSseEvent(event) {
    const payload = (0, outbox_service_1.parseOutboxPayload)(event.payloadJson);
    return {
        id: event.id,
        topic: event.topic,
...
async function streamMutationsSse(req, res) {
    ...
    const watcher = setInterval(async () => {
        try {
            const events = await (0, outbox_service_1.listOutboxEventsSince)(cursor, lastEventId, accountId);
            if (events.length === 0)
                return;
            ...
            logger_1.logger.error({ err, accountId }, 'streamMutationsSse watcher error');
...
async function showMutationsUtama(req, res) {
    try {
        const accounts = await database_1.db.qrisAccount.findMany({
        ...
        logger_1.logger.error({ err }, 'showMutationsUtama error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
async function showMutationsMadera(req, res) {
    try {
        await (0, settlement_service_1.reconcileProcessingMaderaTransfers)().catch((err) => {
            logger_1.logger.warn({ err }, 'showMutationsMadera: unable to reconcile processing madera transfers');
        });
        const accounts = await database_1.db.qrisAccount.findMany({
            where: { status: 'active' },
            orderBy: { code: 'asc' },
        ...
        logger_1.logger.error({ err }, 'showMutationsMadera error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
// ── Mutations JSON (for 2-second frontend poll) ───────────────────────────────
async function showMaderaNobuHistory(req, res) {
    try {
        const accountsRaw = await database_1.db.qrisAccount.findMany({
            where: { status: 'active' },
            orderBy: { code: 'asc' },
        ...
        logger_1.logger.error({ err }, 'showMaderaNobuHistory error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
const capturingProofs = new Set();
async function handleCaptureSettlementProofApi(req, res) {
    try {
        const id = String(req.params.id || '');
        const dir = path_1.default.join(process.cwd(), 'data', 'nobu-proofs');
        const outPath = path_1.default.join(dir, id + '.png');
        if (fs_1.default.existsSync(outPath)) {
            res.json({ ok: true, status: 'ready' });
            return;
        }
        if (capturingProofs.has(id)) {
            res.json({ ok: true, status: 'processing' });
            return;
        }
        const settlement = await database_1.db.settlementRequest.findUnique({ where: { id } });
        if (!settlement || !settlement.qrisAccountId) {
            res.status(404).json({ ok: false, message: 'Transaksi tidak ditemukan.' });
            return;
        }
        const account = await database_1.db.qrisAccount.findUnique({ where: { id: settlement.qrisAccountId } });
        if (!account) {
        ...
        (async () => {
            try {
                const urlRes = await app_orkut_gateway_1.appGateway.fetchMaderaHistoryWebviewUrl(account);
                if (urlRes && urlRes.ok && urlRes.url) {
                    try {
                        fs_1.default.mkdirSync(dir, { recursive: true });
                    }
                    catch (e) { }
                    const shot = await app_orkut_gateway_1.appGateway.captureMaderaHistoryScreenshotAsync(urlRes.url, outPath, account, settlement.amount);
                    logger_1.logger.info({ id, ok: shot && shot.ok, msg: shot && shot.message }, 'captureProof done');
                }
                else {
                    logger_1.logger.warn({ id, msg: urlRes && urlRes.message }, 'captureProof: url gagal');
                }
            }
            catch (err) {
                logger_1.logger.error({ err, id }, 'captureProof background error');
            }
            finally {
                capturingProofs.delete(id);
            }
        })();
        res.json({ ok: true, status: 'processing' });
    }
    catch (err) {
        logger_1.logger.error({ err }, 'handleCaptureSettlementProofApi error');
        res.status(500).json({ ok: false, message: 'Gagal memulai pengambilan bukti.' });
    }
}
async function getSettlementProofApi(req, res) {
    try {
        const id = String(req.params.id || '').replace(/[^A-Za-z0-9_-]/g, '');
        const file = path_1.default.join(process.cwd(), 'data', 'nobu-proofs', id + '.png');
        if (!id || !fs_1.default.existsSync(file)) {
            res.status(404).send('Bukti belum tersedia.');
            return;
        }
        ...
        logger_1.logger.error({ err }, 'getSettlementProofApi error');
        res.status(500).send('Gagal mengunduh bukti.');
    }
}
async function getSettlementProofsListApi(req, res) {
    try {
        const dir = path_1.default.join(process.cwd(), 'data', 'nobu-proofs');
        let ids = [];
        try {
            ids = fs_1.default.readdirSync(dir).filter((f) => f.endsWith('.png')).map((f) => f.slice(0, -4));
        }
        catch (e) { }
...
async function getMaderaNobuWebviewApi(req, res) {
    try {
        const account = await database_1.db.qrisAccount.findUnique({ where: { id: req.params.accountId } });
        if (!account) {
            res.status(404).json({ ok: false, message: 'Akun tidak ditemukan.' });
            return;
        }
        const result = await app_orkut_gateway_1.appGateway.fetchMaderaHistoryWebviewUrl(account);
        res.json(result);
    }
    catch (err) {
        logger_1.logger.error({ err }, 'getMaderaNobuWebviewApi error');
        res.status(500).json({ ok: false, message: 'Gagal mengambil link webview.' });
    }
}
async function getMaderaNobuHistoryApi(req, res) {
    try {
        const account = await database_1.db.qrisAccount.findUnique({ where: { id: req.params.accountId } });
        if (!account) {
            res.status(404).json({ ok: false, message: 'Akun tidak ditemukan.' });
        ...
        const result = await app_orkut_gateway_1.appGateway.fetchMaderaTransactionHistory(account);
        res.json(result);
    }
    catch (err) {
        logger_1.logger.error({ err }, 'getMaderaNobuHistoryApi error');
        res.status(500).json({ ok: false, message: 'Gagal mengambil histori Madera.' });
    }
}
async function getBridgeStatusApi(req, res) {
    try {
        const account = await database_1.db.qrisAccount.findFirst({
            where: { status: 'active', sessionTokenEncrypted: { not: null } },
            orderBy: { code: 'asc' },
        });
        if (!account) {
            res.json({ ok: false, reason: 'no_account', message: 'Tidak ada akun aktif bersession.' });
            return;
        }
        const r = await app_orkut_gateway_1.appGateway.pingEgress(account, 6000);
        res.json({ ok: !!(r && r.ok), latencyMs: r && r.latencyMs, message: (r && r.message) || null });
    }
    catch (err) {
        logger_1.logger.error({ err }, 'getBridgeStatusApi error');
        res.status(500).json({ ok: false, message: 'Gagal cek status bridge.' });
    }
}
async function buildAllAccountsMaderaFromDb(limit) {
    ...
    if (accounts.length === 0)
        return { mutations: [], note: null, balances: {} };
    const balances = {};
    ...
    const settlementsByAccount = new Map();
...
async function buildMaderaRealRowsForAccount(account, source, limit) {
    const rows = await database_1.db.mutation.findMany({
    ...
    const __accSiteName = (0, site_service_1.siteNameForAccount)(account.id);
    return rows.map((m) => {
        const balanceAfter = running;
        const balanceBefore = m.type === 'credit' ? running - m.amount : running + m.amount;
        running = balanceBefore;
        const parsedRaw = parseRawMutationPayload(m.rawDataJson);
...
async function getMutationsJson(req, res) {
    try {
        ...
        if (source === 'qris') {
            ...
            const targetAccount = targetAccountId
                ? await database_1.db.qrisAccount.findUnique({
            ...
            if (targetAccountId &&
                ...
                await (0, orderkuota_report_python_service_1.syncMerchantMutationsFromReportIfStale)(targetAccount, bucket, 15000).catch((err) => {
                    logger_1.logger.warn({ err, accountCode: targetAccount.code, bucket }, 'getMutationsJson: unable to sync report mutations before reading table');
                });
            }
            if (bucket === 'madera' && !targetAccountId) {
                ...
                const maderaAccounts = await database_1.db.qrisAccount.findMany({
            ...
            const mutations = await database_1.db.mutation.findMany({
                where,
                orderBy: { transactionTime: 'desc' },
                take: limit,
                include: {
            ...
            const resolveSite = (0, site_service_1.buildResolver)();
            let presentedMutations = resolvedMutations.map(({ mutation: m, category }) => {
                const parsedRaw = parseRawMutationPayload(m.rawDataJson);
                const description = parsedRaw.description ?? parsedRaw.keterangan ?? '';
                const presentation = parseMutationPresentation(m.rawDataJson, {
                    type: m.type,
            ...
            if (bucket === 'madera' && targetAccount && presentedMutations.length === 0) {
                ...
                if (inferred.mutations.length > 0) {
                    ...
                    const __infSiteName = (0, site_service_1.siteNameForAccount)(targetAccount.id);
            ...
            if (allowLiveUpstreamEnrichment && targetAccountId) {
                ...
                if (liveAccount && fallbackIndex > 0) {
                    const accountIndex = (0, orkut_panel_service_1.resolveOrkutAccountIndex)(liveAccount, fallbackIndex);
                    const webReportPayments = await (0, orkut_web_report_service_1.fetchOrkutWebReportPayments)(liveAccount, accountIndex);
                    if (webReportPayments.length > 0) {
                        presentedMutations = (0, orkut_web_report_service_1.enrichPresentedQrisMutationsWithWebReport)(presentedMutations, webReportPayments);
                    }
                }
                if (liveAccount?.sessionTokenEncrypted) {
                    const detailTargets = presentedMutations
                        .filter((row) => {
                        const rawId = (0, orkut_app_detail_service_1.readPresentedMutationRawId)(row.rawDataJson);
                        if (!rawId)
                            return false;
                        ...
                        rawId: (0, orkut_app_detail_service_1.readPresentedMutationRawId)(row.rawDataJson),
                    }))
                    ...
                    if (detailTargets.length > 0) {
                        ...
                        const detailResults = await Promise.all(detailTargets.map(async (target) => {
                            try {
                                const detail = await app_orkut_gateway_1.appGateway.fetchQrisMutationDetail(liveAccount, target.rawId);
                        ...
                        if (resolvedDetails.length > 0) {
                            presentedMutations = (0, orkut_app_detail_service_1.enrichPresentedQrisMutationsWithAppDetails)(presentedMutations, resolvedDetails.map((item) => item.detail));
                            await Promise.allSettled(resolvedDetails.map(async ({ detail, mutationId, rawDataJson }) => {
                                ...
                                const updatedMutation = await database_1.db.mutation.update({
                                    ...
                                    data: {
                                        ...
                                        rawDataJson: (0, orkut_app_detail_service_1.mergeRawMutationWithAppDetail)(rawDataJson, detail),
            ...
            presentedMutations = (0, orkut_web_report_service_1.dedupePresentedQrisMutations)(presentedMutations);
            const dedupedBucketCounts = presentedMutations.reduce((acc, mutation) => {
        ...
        const walletEntries = await database_1.db.walletLedger.findMany({
        ...
        logger_1.logger.error({ err }, 'getMutationsJson error');
...
async function computeWalletReconcileForAccount(account, walletCategory, physicalBalance) {
    const rows = await database_1.db.mutation.findMany({
...
const maderaReconcileCache = new Map();
const MADERA_RECONCILE_TTL_MS = 60000;
async function computeMaderaReconcileForAccount(account) {
    ...
    let feed;
    try {
        feed = await app_orkut_gateway_1.appGateway.fetchMaderaTransactionHistory(account);
    }
    catch (err) {
    ...
    const mapped = items
        .map((it) => (0, madera_history_service_1.mapMaderaFeedItem)(account.id, it))
        .filter((m) => Boolean(m));
    const hashes = mapped.map((m) => m.rawHash);
    let existing = new Set();
    if (hashes.length > 0) {
        const rows = await database_1.db.mutation.findMany({ where: { rawHash: { in: hashes } }, select: { rawHash: true } });
...
async function getQrisReconcileApi(req, res) {
    try {
        ...
        const accountSelect = { id: true, code: true, merchantName: true, lastQrisBalance: true, lastMainBalance: true, lastMaderaBalance: true, sessionTokenEncrypted: true };
        const computeOne = (acc) => wallet === 'madera'
        ...
        logger_1.logger.error({ err }, 'getQrisReconcileApi error');
        res.status(500).json({ ok: false });
    }
}
// ── Status transaksi Generate QR (untuk polling live PAID/EXPIRED di halaman) ──
async function getGenerateQrStatusApi(req, res) {
    try {
        ...
        logger_1.logger.error({ err }, 'getGenerateQrStatusApi error');
        res.status(500).json({ ok: false });
    }
}
// ── Akun Alias (RBAC per-menu, master-only) ───────────────────────────────────
async function showAkunAlias(req, res) {
    try {
        const accounts = (0, alias_access_service_1.listAccounts)(req.session.user);
        res.render('settings/akun-alias', {
            title: 'Akun Alias',
            menuDefs: alias_access_service_1.MENU_DEFS,
            accounts,
            sites: (0, site_service_1.listSites)(),
        });
    }
    catch (err) {
        logger_1.logger.error({ err }, 'showAkunAlias error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
async function getAliasAccountsApi(req, res) {
    try {
        res.json({ ok: true, accounts: (0, alias_access_service_1.listAccounts)(req.session.user), menuDefs: alias_access_service_1.MENU_DEFS });
    }
    catch (err) {
        logger_1.logger.error({ err }, 'getAliasAccountsApi error');
        res.status(500).json({ ok: false, error: 'Gagal memuat akun' });
    }
}
async function createAliasApi(req, res) {
    try {
        const b = req.body || {};
        await (0, alias_access_service_1.createAlias)({ username: b.username, name: b.name, password: b.password, perms: b.perms || {}, siteScope: b.siteScope });
        res.json({ ok: true });
    }
    catch (err) {
...
async function updateAliasApi(req, res) {
    try {
        ...
        const b = req.body || {};
        await (0, alias_access_service_1.updateAlias)(username, { name: b.name, password: b.password, perms: b.perms, siteScope: b.siteScope });
        res.json({ ok: true });
    }
    catch (err) {
...
async function deleteAliasApi(req, res) {
    try {
        const username = String(req.params.username || '');
        if (username.toLowerCase() === 'harywang') {
        ...
        (0, alias_access_service_1.deleteAlias)(username);
        res.json({ ok: true });
    }
    catch (err) {
...
// ── Settlement ────────────────────────────────────────────────────────────────
async function showSettlement(req, res) {
    try {
        await (0, settlement_service_1.reconcileProcessingMaderaTransfers)().catch((err) => {
            logger_1.logger.warn({ err }, 'showSettlement: unable to reconcile processing madera transfers');
        });
        const [{ settlements, total }, utamaBalance, maderaBalance, doneAgg, rawAccounts] = await Promise.all([
            (0, settlement_service_1.listSettlements)({ limit: 100 }),
            (0, wallet_ledger_service_1.getWalletBalance)('utama'),
            (0, wallet_ledger_service_1.getWalletBalance)('madera'),
            database_1.db.settlementRequest.aggregate({
                where: { status: 'done' },
                _sum: { amount: true },
            }),
            database_1.db.qrisAccount.findMany({
                orderBy: { code: 'asc' },
                select: {
                    id: true,
                    code: true,
        ...
        const _paidAggToday = await database_1.db.transaction.groupBy({ by: ['qrisAccountId'], where: { statusPay: 'paid', paidAt: { gte: _todayWibStart } }, _sum: { finalAmount: true } });
        const _paidTodayMap = {};
        for (const _r of _paidAggToday) {
            if (_r.qrisAccountId)
                _paidTodayMap[_r.qrisAccountId] = _r._sum.finalAmount || 0;
        }
        ...
        const _siteResolve = (0, site_service_1.buildResolver)();
        const accounts = await Promise.all(scopedRawAccounts.map(async (account) => {
            ...
            if (account.sessionTokenEncrypted) {
                try {
                    const [terms, maderaOverview] = await Promise.all([
                        ...
                        app_orkut_gateway_1.appGateway.fetchMaderaTransferOverview(account).catch(() => null),
                    ]);
                    if (terms) {
            ...
            if (nextMaderaBalance === null || nextMaderaBalance === undefined) {
                try {
                    ...
                    logger_1.logger.warn({ err, accountCode: account.code }, 'showSettlement: unable to infer madera balance');
                }
            }
        ...
        logger_1.logger.error({ err }, 'showSettlement error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
// ── Account Transfer API (JSON) ────────────────────────────────────────────────
const AccountTransferSchema = zod_1.z.object({
    fromWallet: zod_1.z.enum(['qris', 'utama', 'madera']),
    toWallet: zod_1.z.enum(['utama', 'madera', 'bank']),
    amount: zod_1.z.number().int().min(1000, 'Minimal transfer Rp 1.000'),
    qrisAccountId: zod_1.z.string().optional(),
    bankCode: zod_1.z.string().max(50).optional(),
    bankAccount: zod_1.z.string().max(50).optional(),
    bankName: zod_1.z.string().max(100).optional(),
    note: zod_1.z.string().max(500).optional(),
});
const SettlementBankInquirySchema = zod_1.z.object({
    fromWallet: zod_1.z.enum(['utama', 'madera']),
    qrisAccountId: zod_1.z.string().min(1, 'Akun merchant wajib dipilih'),
    bankCode: zod_1.z.string().trim().min(1, 'Bank wajib dipilih').max(50),
    bankAccount: zod_1.z.string().trim().min(4, 'Nomor rekening terlalu pendek').max(50),
    amount: zod_1.z.number().int().min(0).max(100000000).optional(),
});
async function handleSettlementBankInquiryApi(req, res) {
    try {
        ...
        const result = await (0, settlement_service_1.inquireSettlementBankAccount)(parsed.data);
        res.json({
            ok: true,
            data: result,
        });
        ...
        logger_1.logger.error({ err }, 'handleSettlementBankInquiryApi error');
        const msg = err instanceof Error ? err.message : 'Gagal melakukan inquiry rekening';
        res.json({ ok: false, error: msg });
    }
}
async function handleAccountTransferApi(req, res) {
    try {
        ...
        const settlementId = await (0, settlement_service_1.createSettlement)({ fromWallet, toWallet, amount, qrisAccountId, bankCode, bankAccount, bankName, note }, req.session.user?.id, req.ip);
        // Process immediately (don't wait for the sweep loop)
        const processResult = await (0, settlement_service_1.processSettlement)(settlementId);
        const settlement = await database_1.db.settlementRequest.findUnique({ where: { id: settlementId } });
        ...
        logger_1.logger.error({ err }, 'handleAccountTransferApi error');
        const msg = err instanceof Error ? err.message : 'Gagal memproses transfer';
        res.status(500).json({ ok: false, error: msg });
    }
}
async function handleRetryAutoPinApi(req, res) {
    try {
        ...
        const pinResult = await app_orkut_gateway_1.appGateway.finalizeMaderaTransferPin(redirectUrl, transferPin, account);
...
async function handleBankListApi(req, res) {
    try {
        ...
        const banks = (await (0, settlement_service_1.listSettlementTransferBanks)(qrisAccountId))
        ...
        logger_1.logger.error({ err }, 'handleBankListApi error');
...
const CreateSettlementFormSchema = zod_1.z.object({
    fromWallet: zod_1.z.enum(['qris', 'utama', 'madera']),
    toWallet: zod_1.z.enum(['utama', 'madera', 'bank']),
    amount: zod_1.z.string().transform((v) => parseInt(v.replace(/\D/g, ''), 10)),
    qrisAccountId: zod_1.z.string().optional(),
    bankCode: zod_1.z.string().max(50).optional(),
    bankAccount: zod_1.z.string().max(50).optional(),
    bankName: zod_1.z.string().max(100).optional(),
...
async function handleCreateSettlement(req, res) {
    try {
        ...
        if (!parsed.success) {
            ...
            res.redirect((0, base_path_1.withBasePath)('/dashboard/settlement', config_1.config.APP_BASE_PATH));
            return;
        }
        const { fromWallet, toWallet, amount, qrisAccountId, bankCode, bankAccount, bankName, note } = parsed.data;
        ...
        await (0, settlement_service_1.createSettlement)({ fromWallet, toWallet, amount, qrisAccountId, bankCode, bankAccount, bankName, note }, req.session.user?.id, req.ip);
        req.session.flash = {
            type: 'success',
        ...
        logger_1.logger.error({ err }, 'handleCreateSettlement error');
...
// ── Login Logs ────────────────────────────────────────────────────────────────
async function showLoginLogs(req, res) {
    try {
        const page = Math.max(1, parseInt(req.query.page) || 1);
        const limit = 100;
        const offset = (page - 1) * limit;
        const [logs, total] = await Promise.all([
            database_1.db.loginLog.findMany({
                orderBy: { createdAt: 'desc' },
                take: limit,
                skip: offset,
                include: { user: { select: { username: true, fullName: true } } },
            }),
            database_1.db.loginLog.count(),
        ]);
        ...
        logger_1.logger.error({ err }, 'showLoginLogs error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
// ── Aliases (stub) ────────────────────────────────────────────────────────────
async function showAliases(req, res) {
    res.render('settings/aliases', { title: 'Alias Pengguna' });
}
// ── Account Settings (Webhook / Web Game / Akun QRIS) ────────────────────────
async function checkWebGamePanelApi(req, res) {
    try {
        const body = req.body || {};
        ...
        const script = path_1.default.join(projectRoot, 'python', 'game_panel.py');
        const sessDir = path_1.default.join(projectRoot, 'data', 'webgame-sessions');
        try {
            fs_1.default.mkdirSync(sessDir, { recursive: true });
        }
        catch (e) { }
        const sessFile = siteId ? path_1.default.join(sessDir, siteId + '.json') : null;
        let cookies = {};
        if (sessFile) {
            try {
                cookies = (JSON.parse(fs_1.default.readFileSync(sessFile, 'utf8')).cookies) || {};
            }
            catch (e) { }
        }
        const input = JSON.stringify({ platform, creds, cookies });
        const proc = (0, child_process_1.spawnSync)(pyBin, [script], { input, encoding: 'utf8', timeout: 45000, maxBuffer: 4 * 1024 * 1024 });
        const out = String(proc.stdout || '');
        const m = out.match(/GAMECHK_JSON_BEGIN\s*([\s\S]*?)\s*GAMECHK_JSON_END/);
        if (!m) {
            logger_1.logger.warn({ stderr: String(proc.stderr || '').slice(-300), siteId }, 'checkWebGamePanelApi: no marker');
        ...
        try {
            ...
            res.json({ ok: false, online: false, message: 'Output cek tidak valid.' });
            return;
        }
        if (sessFile && parsed.cookies && Object.keys(parsed.cookies).length) {
            try {
                fs_1.default.writeFileSync(sessFile, JSON.stringify({ cookies: parsed.cookies, updatedAt: new Date().toISOString() }), 'utf8');
            }
            catch (e) { }
        }
        ...
        logger_1.logger.error({ err }, 'checkWebGamePanelApi error');
        res.status(500).json({ ok: false, online: false, message: 'Error internal saat cek panel.' });
    }
}
async function showAccountSettings(req, res) {
    try {
        const qrisAccounts = await database_1.db.qrisAccount.findMany({
            orderBy: { code: 'asc' },
        ...
        logger_1.logger.error({ err }, 'showAccountSettings error');
        res.render('settings/account', { title: 'Pengaturan Akun', qrisAccounts: [] });
    }
}
// ── QRIS Accounts JSON (for per-account mutation tabs) ────────────────────────
async function getQrisAccountsJson(req, res) {
    try {
        const accounts = await database_1.db.qrisAccount.findMany({
            where: { status: 'active' },
            orderBy: { code: 'asc' },
        ...
        const countRows = await database_1.db.mutation.findMany({
            select: {
                qrisAccountId: true,
                amount: true,
                balanceAfter: true,
        ...
        const dedupedCountRows = (0, orkut_web_report_service_1.dedupePresentedQrisMutations)(countRows.map((row) => ({
        ...
        const countMap = dedupedCountRows.reduce((acc, row) => {
        ...
        logger_1.logger.error({ err }, 'getQrisAccountsJson error');
        res.status(500).json({ ok: false });
    }
}
// ── Generate QR Page ──────────────────────────────────────────────────────────
const RefreshWalletSchema = zod_1.z.object({
    wallet: zod_1.z.enum(['qris', 'utama', 'madera']),
});
async function handleRefreshAccountBalanceApi(req, res) {
    try {
        ...
        const account = await database_1.db.qrisAccount.findUnique({ where: { id: accountId } });
        if (!account) {
        ...
        if (wallet === 'qris' || wallet === 'utama') {
            ...
            if (account.sessionTokenEncrypted) {
                const summary = await app_orkut_gateway_1.appGateway.fetchAccountSummary(account);
                if (wallet === 'qris') {
                ...
                const reportStats = await (0, orderkuota_report_python_service_1.syncMerchantMutationsFromReport)(account, wallet);
            ...
            await (0, settlement_service_1.reconcileProcessingMaderaTransfers)(account.id).catch((err) => {
                logger_1.logger.warn({ err, accountCode: account.code }, 'handleRefreshAccountBalanceApi: unable to reconcile madera transfers');
            });
            let snapshot = null;
            if (account.sessionTokenEncrypted) {
                const [overview, history] = await Promise.all([
                    app_orkut_gateway_1.appGateway.fetchMaderaTransferOverview(account).catch(() => null),
                    app_orkut_gateway_1.appGateway.fetchBalanceHistory(account).catch(() => null),
                ]);
            ...
            if (nextMaderaBalance === null || nextMaderaBalance === undefined) {
                ...
                snapshot = await (0, orkut_panel_service_1.syncOrkutBalanceSnapshot)(account, resolvedIndex);
                if (snapshot) {
        ...
        logger_1.logger.error({ err, accountId: req.params.id }, 'handleRefreshAccountBalanceApi error');
        res.status(500).json({ ok: false, error: 'Gagal me-refresh saldo akun.' });
    }
}
async function showGenerateQr(req, res) {
    try {
        const accountsRaw = await database_1.db.qrisAccount.findMany({
            where: {
                status: 'active',
        ...
        let accounts = (0, site_service_1.attachSiteInfo)(accountsRaw);
        let sites = (0, site_service_1.listSites)();
        // Fase 6: alias-tenant -> hanya site & akun miliknya.
        const _scope = (0, alias_access_service_1.getSiteScopeForUser)(req.session.user);
        if (_scope) {
        ...
        logger_1.logger.error({ err }, 'showGenerateQr error');
        res.status(500).render('error/500', { title: 'Error' });
    }
}
const DashboardGenerateQrSchema = zod_1.z.object({
    accountId: zod_1.z.string().min(1, 'Akun QRIS wajib dipilih'),
    amount: zod_1.z.number().int().min(1, 'Nominal minimal Rp 1').max(10000000, 'Nominal terlalu besar'),
    username: zod_1.z.string().trim().min(1, 'Username wajib diisi').max(100, 'Username terlalu panjang'),
});
async function handleDashboardGenerateQr(req, res) {
    try {
        ...
        if (!accountInScope(req.session.user, parsed.data.accountId)) {
            res.status(403).json({ ok: false, error: 'Akun di luar site Anda.' });
            return;
        }
        const result = await (0, dashboard_generate_qr_service_1.generateDashboardQrTransaction)({
            ...parsed.data,
        ...
        logger_1.logger.error({ err }, 'handleDashboardGenerateQr error');
        res.status(400).json({
            ok: false,
...
async function getQrisTemplate(req, res) {
    try {
        ...
        if (!accountInScope(req.session.user, accountId)) {
            res.status(403).json({ ok: false, error: 'Akun di luar site Anda' });
            return;
        }
        const account = await database_1.db.qrisAccount.findUnique({ where: { id: accountId } });
        ...
        if (account.sessionTokenEncrypted) {
            const result = await app_orkut_gateway_1.appGateway.fetchQrisMerchantTerms(account);
            if (result) {
                ...
                if (account.qrisPayload !== qrisData) {
                    database_1.db.qrisAccount.update({ where: { id: account.id }, data: { qrisPayload: qrisData } }).catch(() => { });
                }
            }
        }
        ...
        if (!qrisData && account.qrisPayload) {
            ...
            logger_1.logger.info({ accountCode: account.code }, 'getQrisTemplate: using stored qrisPayload as fallback');
        }
        if (!qrisData) {
        ...
        logger_1.logger.error({ err }, 'getQrisTemplate error');
        res.status(500).json({ ok: false });
    }
}
