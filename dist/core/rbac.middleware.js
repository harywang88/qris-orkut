...
function requireMenu(menuKey, subKey) {
    return (req, res, next) => {
        ...
        try {
            if (alias_access_service_1.isMasterUser(user))
                return next();
            const perms = alias_access_service_1.getMenuPermsForUser(user);
            const ok = subKey
                ? alias_access_service_1.canDo(perms, menuKey, subKey)
                : alias_access_service_1.canViewMenu(perms, menuKey);
            if (!ok) {
            ...
            // Bila layer alias error, jangan kunci total: izinkan (fail-open) supaya tak memblok kerja.
            return next();
        }
        next();
    };
...
/**
 * Gabungan Akun Alias + izin lama. Izinkan bila:
 *   - master, ATAU
 *   - alias dgn sub-menu (menuPerms) canDo(menuKey, subKey), ATAU
 *   - user Prisma non-master dgn izin dasar (legacyPermission) -> perilaku lama, tanpa regresi.
 * Dipakai route yg dulu hanya requirePermission (mis. /clients, /merchant-qr) supaya alias
 * yang menunya sudah dicentang tidak lagi 403.
 */
function requireMenuOrPermission(menuKey, subKey, legacyPermission) {
    return (req, res, next) => {
        ...
        try {
            ...
            if (user.isAlias) {
                const perms = alias_access_service_1.getMenuPermsForUser(user);
                if (alias_access_service_1.canDo(perms, menuKey, subKey))
                    return next();
                denyAccess(req, res, 'Anda tidak memiliki akses ke menu/fitur ini.');
                return;
            }
