...
exports.dedupePresentedQrisMutations = dedupePresentedQrisMutations;
exports.enrichPresentedQrisMutationsWithWebReport = enrichPresentedQrisMutationsWithWebReport;
exports.fetchOrkutWebReportPayments = fetchOrkutWebReportPayments;
...
function buildDefaultWebReportUrlTemplate() {
    if (!config_1.config.ORKUT_BALANCE_BASE_URL)
        return null;
    const trimmed = config_1.config.ORKUT_BALANCE_BASE_URL.replace(/\/+$/, '');
    return `${trimmed}/mutasi-qr?action=fetch_web&account={account}&cache=0&_={ts}`;
}
function getWebReportUrlTemplate() {
    return config_1.config.ORKUT_QRIS_WEB_REPORT_URL_TEMPLATE || buildDefaultWebReportUrlTemplate();
}
function buildWebReportUrl(accountIndex) {
    ...
    try {
        ...
        logger_1.logger.warn({ accountIndex, resolved }, 'web-report: invalid URL template result');
        return null;
    }
}
function parseWebReportPayload(payload) {
    ...
    return paymentsValue
        ...
        if (!amount || amount <= 0)
            return null;
        const transactionTime = parseTimestamp(raw.timestamp) ??
...
function buildHeaders(account, url) {
    const headers = {
    ...
    const shouldSendOrderkuotaCookies = host === 'orderkuota.com' ||
        host.endsWith('.orderkuota.com');
    if (shouldSendOrderkuotaCookies && account.cookiesEncrypted) {
        try {
            headers.Cookie = (0, encryption_1.decrypt)(account.cookiesEncrypted);
        }
        catch (err) {
            logger_1.logger.warn({ accountCode: account.code, err }, 'web-report: failed to decrypt cookies');
        }
    }
    return headers;
}
function parseRawMutationId(rawDataJson) {
    try {
...
function parsePresentedTime(row) {
...
function scoreWebReportMatch(row, payment) {
    if (payment.amount !== row.amount)
        return -1;
    if (payment.statusCode !== row.statusCode.toUpperCase())
        return -1;
    let score = 0;
    if (payment.balanceAfter !== null && payment.balanceAfter === row.balanceAfter)
        score += 3;
...
function pickPreferredMutation(current, candidate) {
    const currentScore = (current.rrn ? 4 : 0) +
        (hasSeconds(current.displayTime) ? 2 : 0) +
        (current.matched ? 1 : 0) +
        (current.senderName && current.senderName !== 'Tidak terbaca' ? 1 : 0);
    const candidateScore = (candidate.rrn ? 4 : 0) +
...
function buildDedupKey(row) {
    ...
    if (rawId)
        return `${row.accountCode}|${rawId}`;
    if (row.rrn)
        return `${row.accountCode}|rrn|${row.rrn}|${row.amount}`;
    return [
        row.accountCode,
        row.statusCode,
        row.amount,
...
function dedupePresentedQrisMutations(rows) {
    const seen = new Map();
...
function enrichPresentedQrisMutationsWithWebReport(rows, payments) {
    ...
    return rows.map((row) => {
        ...
        return {
            ...
            displayTime: match.timestamp && (!hasSeconds(row.displayTime) || hasSeconds(match.timestamp))
                ? match.timestamp
                : row.displayTime,
...
async function fetchOrkutWebReportPayments(account, accountIndex) {
    ...
    try {
        ...
        if (!response.ok) {
            logger_1.logger.debug({ accountCode: account.code, accountIndex, status: response.status, url }, 'web-report: request returned non-200 status');
            return [];
        }
        const payload = await response.json();
        const payments = parseWebReportPayload(payload);
        logger_1.logger.debug({ accountCode: account.code, accountIndex, total: payments.length }, 'web-report: fetched QRIS payments');
        return payments;
    }
    catch (err) {
        logger_1.logger.debug({ accountCode: account.code, accountIndex, err }, 'web-report: request failed');
        return [];
    }
}
//# sourceMappingURL=orkut-web-report.service.js.map