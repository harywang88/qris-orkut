...
exports.appGateway = exports.AppOrkutGateway = exports.AppOrkutRateLimitError = exports.APP_QRIS_RATE_LIMIT_COOLDOWN_MS = void 0;
...
const UI_MODE = 'light';
const MAX_PAGES = 10;
exports.APP_QRIS_RATE_LIMIT_COOLDOWN_MS = 5 * 60 * 1000;
const APP_PROXY_URL = process.env.ORKUT_PROXY_URL?.trim() || '';
const APP_PROXY_AGENT = APP_PROXY_URL ? new undici_1.ProxyAgent(APP_PROXY_URL) : undefined;
...
const NOBU_PROXY_AGENT = NOBU_PROXY_URL ? new undici_1.ProxyAgent(NOBU_PROXY_URL) : undefined;
/**
 ...
 */
function resolveBankCode(bankName, accountCode) {
    ...
    if (BANK_SWIFT_MAP[bankName])
        return BANK_SWIFT_MAP[bankName];
...
function resolveBankRouting(bankName, accountCode) {
...
class AppOrkutRateLimitError extends Error {
    constructor(message = 'OrderKuota QRIS sedang rate limited', retryAfterMs = exports.APP_QRIS_RATE_LIMIT_COOLDOWN_MS) {
        super(message);
        this.name = 'AppOrkutRateLimitError';
        this.retryAfterMs = retryAfterMs;
    }
}
exports.AppOrkutRateLimitError = AppOrkutRateLimitError;
// ── Helpers ────────────────────────────────────────────────────────────────────
/**
 ...
 */
function buildSignature(timestamp, tokenSecret) {
    return crypto_1.default.createHmac('sha512', tokenSecret).update(timestamp).digest('hex');
}
function buildOfficialSignature(body) {
    const timestamp = body.get('request_time')?.trim() || '';
    const authToken = body.get('auth_token')?.trim() || '';
    if (!timestamp || !authToken || timestamp.length !== 13)
        return null;
...
async function signedAppFetch(input, init = {}, useNobuEgress = false) {
    const headers = new undici_1.Headers(init.headers);
...
function parseOptionalAmount(val) {
...
function asRecordArray(val) {
    if (!Array.isArray(val))
        return [];
    return val.filter((item) => Boolean(item) && typeof item === 'object');
}
...
function parseSendMoneyStatus(value) {
    const raw = readString(value).toUpperCase();
    if (raw === 'DONE' || raw === 'SUKSES' || raw === 'SUCCESS')
        return 'done';
    if (raw === 'PENDING' || raw === 'PROSES' || raw === 'PROCESSING' || raw === 'WAITING')
        return 'processing';
    return 'failed';
}
function parseSendMoneyDetail(raw, fallbackMessage) {
...
function normalizeBankInquiryMessage(message, sourceWallet) {
...
function isQrisRateLimited(status, json) {
    if (status === 469)
        return true;
...
function normalizeAppStatus(value) {
    const raw = readString(value).toUpperCase();
    if (raw === 'DEBIT' || raw === 'OUT')
        return 'OUT';
    return 'IN';
}
function classifyBalanceHistoryWallet(description) {
...
function resolveAppRequestContext(account) {
    if (!account.sessionTokenEncrypted)
        return null;
    try {
        const parsed = parseSessionToken((0, encryption_1.decrypt)(account.sessionTokenEncrypted));
        let appRegId = parsed.accountId;
        if (account.cookiesEncrypted) {
            try {
                appRegId = (0, encryption_1.decrypt)(account.cookiesEncrypted);
            }
            catch {
        ...
        logger_1.logger.warn({ accountCode: account.code, err }, 'app-orkut: failed to resolve request context');
        return null;
    }
}
/**
 ...
 */
function parseMutationEntry(raw, walletCat = 'qris') {
    // ── Amount ────────────────────────────────────────────────────────────────
    ...
    if (amount === 0)
        return null;
    // ── Balance ───────────────────────────────────────────────────────────────
    const balanceAfter = parseAmount(raw.saldo ?? raw.saldo_akhir ?? raw.balance_after ?? raw.saldo_sekarang ?? 0);
    ...
    const rawHash = crypto_1.default.createHash('sha256').update(hashSource).digest('hex');
    return {
        amount,
...
class AppOrkutGateway {
    ...
    async fetchAccountSummary(account) {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, preview: text.slice(0, 200) }, 'app-orkut account-summary: non-JSON response');
                return { mainBalance: null, qrisBalance: null };
            }
            if (!res.ok || json.success === false) {
                if (isQrisRateLimited(res.status, json)) {
                    throw new AppOrkutRateLimitError(readApiMessage(json) || 'OrderKuota menahan akses account summary sementara');
                }
                logger_1.logger.warn({ accountCode: account.code, status: res.status, json }, 'app-orkut account-summary: error response');
                return { mainBalance: null, qrisBalance: null };
            }
            const acctWrapper = json.account;
            const acctData = (acctWrapper?.results ?? acctWrapper);
            return {
            ...
            logger_1.logger.error({ err, accountCode: account.code }, 'app-orkut: fetchAccountSummary error');
            return { mainBalance: null, qrisBalance: null };
        }
    }
    /**
     ...
     */
    async fetchMutationsAndBalance(account, options = {}) {
        const empty = { mutations: [], balance: { mainBalance: null, qrisBalance: null } };
        const hasIncrementalHints = !!options.fromTime
        ...
        let parsed;
        try {
            const rawToken = (0, encryption_1.decrypt)(account.sessionTokenEncrypted);
            parsed = parseSessionToken(rawToken);
        }
        catch (err) {
            logger_1.logger.warn({ accountCode: account.code, err }, 'app-orkut: failed to decrypt/parse session token');
            return empty;
        }
        ...
        const allMutations = this._parsePage(firstPage, account.code);
        const totalPages = Math.min(typeof firstPage.pages === 'number' ? firstPage.pages : 1, MAX_PAGES);
        ...
        logger_1.logger.debug({ accountCode: account.code, pages: totalPages, total: allMutations.length }, 'app-orkut: fetched mutations');
        return { mutations: allMutations, balance };
    }
    // ── Private helpers ────────────────────────────────────────────────────────
    async fetchIncrementalMutationsAndBalance(account, options = {}) {
        const empty = { mutations: [], balance: { mainBalance: null, qrisBalance: null } };
        ...
        const firstResult = await this._fetchPage(url, 1, context.parsed, context.appRegId, context.phoneUuid, account.code, { fromDate });
        if (!firstResult)
            return empty;
        const { pageData: firstPage, balance } = firstResult;
        const allMutations = [];
        let encounteredKnownHash = this._appendFreshMutations(allMutations, this._parsePage(firstPage, account.code), knownRawHashes, stopOnKnown);
        const totalPages = Math.min(typeof firstPage.pages === 'number' ? firstPage.pages : 1, maxPages);
        for (let page = 2; page <= totalPages; page++) {
            if (encounteredKnownHash && stopOnKnown)
                break;
            const result = await this._fetchPage(url, page, context.parsed, context.appRegId, context.phoneUuid, account.code, { fromDate });
            if (!result)
                break;
            encounteredKnownHash = this._appendFreshMutations(allMutations, this._parsePage(result.pageData, account.code), knownRawHashes, stopOnKnown) || encounteredKnownHash;
        }
    ...
    async _fetchPage(url, page, parsed, appRegId, phoneUuid, accountCode, filters = {}) {
        const timestamp = String(Date.now());
        const signature = buildSignature(timestamp, parsed.tokenSecret);
        const body = new URLSearchParams({
            app_reg_id: appRegId,
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode, status: res.status, preview: text.slice(0, 300) }, 'app-orkut: non-JSON response — cookies/token mungkin kadaluarsa');
                return null;
            }
            if (!res.ok) {
                if (isQrisRateLimited(res.status, json)) {
                ...
                logger_1.logger.warn({ accountCode, status: res.status, json }, 'app-orkut: HTTP error');
                return null;
            }
            if (json.success === false) {
                logger_1.logger.warn({ accountCode, json }, 'app-orkut: API success=false — token mungkin tidak valid');
                return null;
            }
            ...
            const historyData = json.qris_history;
            ...
            logger_1.logger.error({ err, accountCode }, 'app-orkut: fetch error');
            return null;
        }
    }
    _parsePage(pageData, accountCode, walletCat = 'qris') {
        ...
        for (const raw of results) {
            if (!raw || typeof raw !== 'object')
                continue;
            const m = parseMutationEntry(raw, walletCat);
            if (m)
                mutations.push(m);
            else
                logger_1.logger.debug({ accountCode, raw }, 'app-orkut: skipped zero-amount entry');
        }
        return mutations;
    }
     ...
     */
    _appendFreshMutations(target, incoming, knownRawHashes, stopOnKnown) {
    ...
    async fetchBalanceHistory(account) {
        const empty = { mutations: [], mainBalance: null };
        const ctx = resolveAppRequestContext(account);
        if (!ctx)
            return empty;
        const { parsed, appRegId, phoneUuid } = ctx;
        ...
        if (!first)
            return empty;
        const allMutations = this._parsePage(first.pageData, account.code, 'utama');
        const mainBalance = first.mainBalance;
        const totalPages = Math.min(typeof first.pageData.pages === 'number' ? first.pageData.pages : 1, MAX_PAGES);
        for (let page = 2; page <= totalPages; page++) {
        ...
        logger_1.logger.debug({ accountCode: account.code, total: allMutations.length, mainBalance }, 'app-orkut: fetched balance history');
        return { mutations: allMutations, mainBalance };
    }
    async _fetchBalancePage(page, parsed, appRegId, phoneUuid, accountCode) {
        const timestamp = String(Date.now());
        const signature = buildSignature(timestamp, parsed.tokenSecret);
        const body = new URLSearchParams({
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode, preview: text.slice(0, 200) }, 'app-orkut balance-hist: non-JSON response');
                return null;
            }
            if (!res.ok || json.success === false) {
                logger_1.logger.warn({ accountCode, status: res.status }, 'app-orkut balance-hist: error response');
                return null;
            }
            // Extract mainBalance dari balance.results.balance
            const balWrapper = json.balance;
            const balData = (balWrapper?.results ?? balWrapper);
            const mainBalance = balData ? (parseAmount(balData.balance ?? null)) || null : null;
            const historyData = json.balance_history;
            return { pageData: historyData ?? {}, mainBalance };
        }
        catch (err) {
            logger_1.logger.error({ err, accountCode }, 'app-orkut balance-hist: fetch error');
            return null;
        }
    }
     ...
     */
    async fetchQrisMerchantTerms(account) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx)
            return null;
        const { parsed, appRegId, phoneUuid } = ctx;
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, preview: text.slice(0, 200) }, 'app-orkut qris-terms: non-JSON');
                return null;
            }
            if (!res.ok || json.success === false) {
                if (isQrisRateLimited(res.status, json)) {
                ...
                logger_1.logger.warn({ accountCode: account.code, status: res.status }, 'app-orkut qris-terms: error response');
                return null;
            }
            const terms = json.qris_merchant_terms;
            const results = (terms?.results ?? terms);
            const qrisData = results?.qris_data;
            if (!qrisData) {
                logger_1.logger.warn({ accountCode: account.code, results }, 'app-orkut qris-terms: qris_data not found');
                return null;
            }
            return {
                qrisData,
                min: parseAmount(results?.min ?? 1),
                max: parseAmount(results?.max ?? 10000000),
                expired: typeof results?.expired === 'number' ? results.expired : 300,
            };
        }
        catch (err) {
            if (err instanceof AppOrkutRateLimitError) {
            ...
            logger_1.logger.error({ err, accountCode: account.code }, 'app-orkut: fetchQrisMerchantTerms error');
            return null;
        }
    }
    async fetchQrisWithdrawTerms(account) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx)
            return null;
        const { parsed, appRegId, phoneUuid } = ctx;
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, preview: text.slice(0, 200) }, 'app-orkut qris-withdraw-terms: non-JSON');
                return null;
            }
            if (!res.ok || json.success === false) {
                if (isQrisRateLimited(res.status, json)) {
                ...
                logger_1.logger.warn({ accountCode: account.code, status: res.status, json }, 'app-orkut qris-withdraw-terms: error response');
                return null;
            }
            const accountWrapper = json.account;
            const accountData = (accountWrapper?.results ?? accountWrapper);
            const termsWrapper = json.qris_withdraw_terms;
            const termsData = (termsWrapper?.results ?? termsWrapper);
            if (!termsData) {
                logger_1.logger.warn({ accountCode: account.code, json }, 'app-orkut qris-withdraw-terms: results not found');
                return null;
            }
            return {
            ...
            logger_1.logger.error({ err, accountCode: account.code }, 'app-orkut: fetchQrisWithdrawTerms error');
            return null;
        }
    }
    async withdrawQris(account, amount) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx) {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, preview: text.slice(0, 300) }, 'app-orkut qris-withdraw: non-JSON');
                throw new Error('Response OrderKuota tidak valid saat tarik QRIS.');
            }
            const withdrawWrapper = json.qris_withdraw;
            ...
            if (!res.ok || json.success === false) {
                if (isQrisRateLimited(res.status, json) || nestedMessage.toLowerCase().includes('coba kembali 5 menit')) {
                    throw new AppOrkutRateLimitError(nestedMessage || readApiMessage(json) || 'OrderKuota menahan akses tarik QRIS sementara');
                }
                return {
                    success: false,
            ...
            logger_1.logger.error({ err, accountCode: account.code, amount }, 'app-orkut: withdrawQris error');
            throw err;
        }
    }
    async topupMadera(account, amount) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx) {
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, preview: text.slice(0, 300) }, 'app-orkut madera-topup: non-JSON');
                throw new Error('Response OrderKuota tidak valid saat topup Madera.');
            }
            const wrapper = json.madera_topup;
            const results = (wrapper?.results ?? wrapper);
            ...
            const success = typeof wrapper?.success === 'boolean'
                ? wrapper.success
                : Boolean(json.success);
            const message = nestedMessage || readApiMessage(json) || (success ? 'Topup Madera berhasil diproses.' : 'Topup Madera gagal diproses.');
            return { success, message, detailsId, raw: json };
        }
        catch (err) {
            logger_1.logger.error({ err, accountCode: account.code, amount }, 'app-orkut: topupMadera error');
            throw err;
        }
    }
    /**
     ...
     */
    async fetchMaderaBankList(account) {
    ...
    async fetchMaderaTransferOverview(account) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx)
            return null;
        const { parsed, appRegId, phoneUuid } = ctx;
        ...
        try {
            ...
            if (!banks || Object.keys(banks).length === 0) {
                logger_1.logger.warn({ accountCode: account.code }, 'app-orkut fetchMaderaBankList: empty bank list');
                return null;
            }
            const parsed_banks = {};
            ...
            logger_1.logger.info({ accountCode: account.code, bankCount: Object.keys(parsed_banks).length }, 'app-orkut fetchMaderaBankList: bank list cached');
            return {
            ...
            logger_1.logger.warn({ err, accountCode: account.code }, 'app-orkut fetchMaderaBankList: failed');
            return null;
        }
    }
    async inquireBankAccount(account, params) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx) {
        ...
        const timestamp = String(Date.now());
        const signature = buildSignature(timestamp, parsed.tokenSecret);
        const minInquiryAmount = params.sourceWallet === 'madera' ? 10000 : 1000;
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, sourceWallet: params.sourceWallet, preview: text.slice(0, 300) }, 'app-orkut bank-inquiry: non-JSON');
            ...
            if (!res.ok || json.success === false) {
                const message = normalizeBankInquiryMessage(readApiMessage(json) || 'Inquiry rekening gagal diproses.', params.sourceWallet);
                return {
                    success: false,
                    sourceWallet: params.sourceWallet,
            ...
            if (params.sourceWallet === 'madera') {
                const wrapper = json.madera_transfer;
                const results = (wrapper?.results ?? wrapper);
                const success = typeof wrapper?.success === 'boolean'
                    ? wrapper.success
                    : Boolean(json.success);
            ...
            const detail = parseSendMoneyDetail(resultsWrapper?.results ?? wrapper?.results ?? resultsWrapper, readString(wrapper?.message) || readApiMessage(json) || null);
            ...
            logger_1.logger.error({ err, accountCode: account.code, sourceWallet: params.sourceWallet, bankCode: params.bankCode }, 'app-orkut: inquireBankAccount error');
            throw err;
        }
    }
    async runMaderaTransferRequest(account, entries, timeoutMs = 20000) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx) {
        ...
        try {
            ...
            logger_1.logger.warn({ accountCode: account.code, preview: text.slice(0, 300) }, 'app-orkut madera-transfer: non-JSON response');
            throw new Error('Response OrderKuota tidak valid saat transfer Madera.');
        }
    }
    parseSimpleHtmlInputs(html) {
        const inputs = [];
    ...
    buildBrowserHeaders(account) {
        ...
        if (account) {
            ...
            if (cookieSource) {
                try {
                    headers.Cookie = (0, encryption_1.decrypt)(cookieSource);
                }
                catch {
    ...
    async finalizeMaderaTransferPin(redirectUrl, pin, account) {
        ...
        if (!m) {
            logger_1.logger.warn({ accountCode: account?.code, status: res.status, stderr: stderr.slice(0, 400) }, 'app-orkut finalizeMaderaTransferPin: headless no result');
        ...
        logger_1.logger.info({ accountCode: account?.code, success: parsed.success, message: parsed.message }, 'app-orkut finalizeMaderaTransferPin: headless result');
        return {
            success: Boolean(parsed.success),
            message: parsed.message || (parsed.success ? 'Konfirmasi PIN berhasil (headless).' : 'Konfirmasi PIN belum berhasil (headless).'),
            raw: { step: 'headless', result: parsed },
        };
    }
    async fetchSendMoneyHistory(account, id) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx)
            return null;
        const { parsed, appRegId, phoneUuid } = ctx;
        ...
        try {
            ...
            if (!res.ok || json.success === false || parseBoolean(wrapper?.success) === false) {
                logger_1.logger.warn({ accountCode: account.code, id, message }, 'app-orkut send-money history: error response');
                return null;
            }
            ...
            logger_1.logger.error({ err, accountCode: account.code, id }, 'app-orkut: fetchSendMoneyHistory error');
            return null;
        }
    }
    async transferBankFromUtama(account, params) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx) {
        ...
        const runRequest = async (entries, timeoutMs = 20000) => {
            const timestamp = String(Date.now());
            const signature = buildSignature(timestamp, parsed.tokenSecret);
            const body = new URLSearchParams({
                request_time: timestamp,
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, preview: text.slice(0, 300) }, 'app-orkut send-money: non-JSON response');
        ...
        try {
            ...
            const checkedDetail = parseSendMoneyDetail(checkResultsWrapper?.results ?? checkWrapper?.results ?? checkResultsWrapper, readString(checkWrapper?.message) || readApiMessage(checkJson) || null);
            ...
            logger_1.logger.error({ err, accountCode: account.code, amount: params.amount, bankCode: params.bankCode }, 'app-orkut: transferBankFromUtama error');
            throw err;
        }
    }
    async transferBankFromMadera(account, params) {
        ...
        if (!inquiry.sessionId) {
            logger_1.logger.warn({ accountCode: account.code, bankCode: params.bankCode }, 'app-orkut transferBankFromMadera: no session_id from inquiry, proceeding anyway');
        }
        try {
            ...
            let finalStatus = success ? 'done' : 'failed';
            ...
            if (redirectUrl) {
                ...
                if (params.pin) {
                    ...
                    const pinOutcome = pinResult.raw?.result?.outcome;
            ...
            logger_1.logger.error({ err, accountCode: account.code, bankCode: params.bankCode, amount: params.amount }, 'app-orkut: transferBankFromMadera error');
            throw err;
        }
    }
    async fetchQrisMutationDetail(account, mutationId) {
        const ctx = resolveAppRequestContext(account);
        if (!ctx)
            return null;
        const { parsed, appRegId, phoneUuid } = ctx;
        ...
        try {
            ...
            try {
                ...
                logger_1.logger.warn({ accountCode: account.code, mutationId, preview: text.slice(0, 200) }, 'app-orkut qris-details: non-JSON response');
                return null;
            }
            if (!res.ok || json.success === false) {
                if (isQrisRateLimited(res.status, json)) {
                ...
                logger_1.logger.warn({ accountCode: account.code, mutationId, status: res.status }, 'app-orkut qris-details: error response');
                return null;
            }
            const wrapper = json.qris_details;
            const results = (wrapper?.results ?? wrapper);
            if (!results) {
                logger_1.logger.warn({ accountCode: account.code, mutationId }, 'app-orkut qris-details: results not found');
                return null;
            }
            return {
            ...
            logger_1.logger.error({ err, accountCode: account.code, mutationId }, 'app-orkut: fetchQrisMutationDetail error');
            return null;
        }
    }
...
exports.AppOrkutGateway = AppOrkutGateway;
exports.appGateway = new AppOrkutGateway();
//# sourceMappingURL=app-orkut.gateway.js.map