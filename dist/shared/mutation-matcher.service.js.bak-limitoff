...
exports.tryMatchMutation = tryMatchMutation;
exports.fetchUnmatchedMutations = fetchUnmatchedMutations;
const database_1 = require("../config/database");
const logger_1 = require("../config/logger");
const mutation_ingest_service_1 = require("./mutation-ingest.service");
const MATCH_CLOCK_SKEW_MS = 30000;
const MATCH_EXPIRY_GRACE_MS = 60000;
function getMutationEffectiveTime(mutation) {
    return mutation.transactionTime ?? mutation.createdAt;
}
function buildTransactionWindow(effectiveTime) {
    return {
...
function isSuspiciousPaidTransaction(transaction) {
    if (!transaction.rrn || !transaction.paidAt)
        return true;
 ...
 */
async function tryMatchMutation(mutation) {
    try {
        const result = await database_1.db.$transaction(async (tx) => {
            ...
            if (effectiveTime.getTime() > targetTransaction.expiresAt.getTime() + MATCH_EXPIRY_GRACE_MS) {
                return { matched: false, reason: 'transaction_expired' };
            }
            ...
            await (0, mutation_ingest_service_1.publishMutationUpdated)(updatedMutation, 'matched', tx);
            ...
            return { matched: true, transactionId: targetTransaction.id, qrId: targetTransaction.qrId };
        });
        if (result.matched) {
            ...
            logger_1.logger.warn({ mutationId: mutation.id, amount: mutation.amount, reason: result.reason }, 'Mutation could not be matched');
        }
        return result;
    }
    catch (err) {
        logger_1.logger.error({ err, mutationId: mutation.id }, 'tryMatchMutation error');
        throw err;
    }
}
/**
 ...
 */
async function fetchUnmatchedMutations(limit = 20) {
    return database_1.db.mutation.findMany({
        where: {
            type: 'credit',
            walletCategory: 'qris',
...
//# sourceMappingURL=mutation-matcher.service.js.map