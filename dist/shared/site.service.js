...
function withColor(s) {
    return Object.assign({}, s, { color: normalizeColor(s.color) || defaultColorFor(s.id) });
}
// ── Site CRUD ────────────────────────────────────────────────────────────────
function listSites() {
    return loadSites()
...
function createSite(name, color) {
...
function updateSite(id, patch) {
    const opts = (typeof patch === 'string') ? { name: patch } : (patch || {});
    const list = loadSites();
