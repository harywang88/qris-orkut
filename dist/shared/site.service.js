...
const SITES_FILE = path_1.default.join(DATA_DIR, 'sites.json');
const MAP_FILE = path_1.default.join(DATA_DIR, 'account-sites.json');
function ensureDir() {
    try {
...
function readJson(file, fallback) {
    try {
        ...
        logger_1.logger.error({ err, file }, 'site.service: gagal baca ' + file);
        return fallback;
    }
}
...
function createSite(name, color) {
    ...
    const id = 'site_' + crypto_1.default.randomBytes(6).toString('hex');
    const site = {
        id,
        name: clean,
