...
type Pm2Process = {
  ...
  pm2_env?: {
    status?: string;
    pm_uptime?: number | string;
    restart_time?: number | string;
    exec_mode?: string;
...
const PM2_HOME = (process.env.PM2_HOME ?? path.join(process.cwd(), 'data', 'pm2')).trim();
const PM2_WRAPPER = (process.env.TELEGRAM_CONTROL_PM2_WRAPPER ?? 'scripts\\pm2.cmd').trim();
const STACK_TARGETS = (
  process.env.TELEGRAM_CONTROL_STACK_TARGETS ?? 'qris-app,qris-worker'
)
  .split(',')
  .map((value) => value.trim())
  .filter(Boolean);
const STACK_TARGETS_ARG = STACK_TARGETS.join(' ');
const ALL_STACK_TARGETS = (
  process.env.TELEGRAM_CONTROL_ALL_STACK_TARGETS ?? 'qris-app,qris-worker,qris-telegram-bot'
)
  .split(',')
  .map((value) => value.trim())
  .filter(Boolean);
...
const START_COMMAND = (
  process.env.TELEGRAM_CONTROL_START_COMMAND ??
  `${PM2_WRAPPER} start ecosystem.config.cjs --only qris-app,qris-worker`
).trim();
const STOP_COMMAND = (
  process.env.TELEGRAM_CONTROL_STOP_COMMAND ??
  `${PM2_WRAPPER} stop ${STACK_TARGETS_ARG}`
).trim();
const RESTART_COMMAND = (
  process.env.TELEGRAM_CONTROL_RESTART_COMMAND ??
  `${PM2_WRAPPER} restart ${STACK_TARGETS_ARG}`
).trim();
const STATUS_COMMAND = (
  process.env.TELEGRAM_CONTROL_STATUS_COMMAND ??
  `${PM2_WRAPPER} list --no-color`
).trim();
const PM2_LIST_COMMAND = (
  process.env.TELEGRAM_CONTROL_PM2_LIST_COMMAND ??
  `${PM2_WRAPPER} jlist`
).trim();
const PM2_RAW_COMMAND = (
  process.env.TELEGRAM_CONTROL_PM2_RAW_COMMAND ??
  `${PM2_WRAPPER} list --no-color`
).trim();
const START_ALL_COMMAND = (
  process.env.TELEGRAM_CONTROL_START_ALL_COMMAND ??
  `${PM2_WRAPPER} start ecosystem.config.cjs`
).trim();
const STOP_ALL_COMMAND = (
  process.env.TELEGRAM_CONTROL_STOP_ALL_COMMAND ??
  `${PM2_WRAPPER} stop ${ALL_STACK_TARGETS_ARG}`
).trim();
const RESTART_ALL_COMMAND = (
  process.env.TELEGRAM_CONTROL_RESTART_ALL_COMMAND ??
  `${PM2_WRAPPER} restart ${ALL_STACK_TARGETS_ARG}`
).trim();
...
function trimOutput(value: string, limit = 3500): string {
  const text = value.trim();
  if (!text) return '(tanpa output)';
...
function statusIcon(status: string): string {
  ...
  if (normalized === 'launching') return '[BOOT]';
  if (normalized === 'errored') return '[ERR]';
  return '[?]';
}
...
async function sendPanel(chatId: number, text?: string): Promise<void> {
...
async function runCommand(command: string, options: RunCommandOptions = {}): Promise<CommandResult> {
  ...
  try {
    ...
    const rawOutput = sanitizeOutput(
...
function buildPm2StatusText(list: Pm2Process[]): string {
...
async function fetchPm2Snapshot(): Promise<{ success: boolean; text: string }> {
  ...
  try {
    ...
    const list = JSON.parse(payload) as Pm2Process[];
...
async function executeAllAction(
  action: 'start-all' | 'stop-all' | 'restart-all',
): Promise<{ success: boolean; output: string; command: string }> {
  const commandMap: Record<typeof action, string> = {
...
async function sendTransientAllServicesNotice(
  chatId: number,
  title: string,
  command: string,
): Promise<void> {
...
async function handleMessage(update: TelegramUpdate): Promise<void> {
  ...
  if (isCommand(msg.text, 'pm2raw')) {
    ...
    await sendCommandOutput(
      msg.chat.id,
      `PM2 RAW ${result.success ? 'SUKSES' : 'GAGAL'}`,
      PM2_RAW_COMMAND,
      result.output,
    );
  }
...
async function handleCallbackQuery(update: TelegramUpdate): Promise<void> {
  ...
  if (action === 'start-all' || action === 'stop-all' || action === 'restart-all') {
    ...
    await sendCommandOutput(
      chatId,
      `${actionLabel} ${result.success ? 'SUKSES' : 'GAGAL'}`,
      result.command,
      result.output,
    );
    return;
  ...
  const result = await executeAction(action as 'start' | 'stop' | 'restart');
  await sendCommandOutput(
    chatId,
    `${action.toUpperCase()} ${result.success ? 'SUKSES' : 'GAGAL'}`,
    result.command,
    result.output,
  );
}
