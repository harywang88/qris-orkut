 ...
 */
export async function findClientByWidgetKey(widgetKey: string) {
  if (!widgetKey || widgetKey.length < 16) return null;
...
export function isOriginAllowed(
  ...
  const candidate =
    normaliseOrigin(origin ?? '') || normaliseOrigin(referer ?? '');
