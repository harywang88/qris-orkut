...
import { recordWalletEntry, getWalletBalance } from './wallet-ledger.service';
import { writeAuditLog } from './audit-log.service';
import { appGateway } from './gateways/app-orkut.gateway';
import { z } from 'zod';
import { decrypt } from '../core/encryption';
import {
  fetchOrkutTransferBanks,
  inquireOrkutBankAccount,
  syncOrkutBalanceSnapshot,
  transferOrkutBankFromMadera,
} from './orkut-panel.service';
import { createHash } from 'crypto';

export const CreateSettlementSchema = z.object({
  fromWallet: z.enum(['qris', 'utama', 'madera']),
  toWallet: z.enum(['utama', 'madera', 'bank']),
  amount: z.number().int().min(1, 'Jumlah minimal Rp 1'),
...
function parseRawSettlementNoteMarkers(note: string | null | undefined): {
...
function buildSettlementNote(note: string | null | undefined, markers: {
  fee?: number | null;
  total?: number | null;
  redirectUrl?: string | null;
}): string | null {
  const parsed = parseRawSettlementNoteMarkers(note);
  const parts = [parsed.cleanNote].filter(Boolean) as string[];
  if (markers.fee !== undefined && markers.fee !== null) parts.push(`[[FEE:${markers.fee}]]`);
  else if (parsed.fee !== null) parts.push(`[[FEE:${parsed.fee}]]`);
  if (markers.total !== undefined && markers.total !== null) parts.push(`[[TOTAL:${markers.total}]]`);
  else if (parsed.total !== null) parts.push(`[[TOTAL:${parsed.total}]]`);
  if (markers.redirectUrl !== undefined && markers.redirectUrl !== null) parts.push(`[[REDIRECT_URL:${markers.redirectUrl}]]`);
  else if (parsed.redirectUrl) parts.push(`[[REDIRECT_URL:${parsed.redirectUrl}]]`);
  return parts.length > 0 ? parts.join('\n') : null;
}

function classifyMaderaHistoryDescription(description: string): boolean {
...
function parseHistoryAmount(value: unknown): number | null {
...
function parseHistoryDate(value: unknown): Date | null {
  ...
  const d = new Date(
    ...
    Number.parseInt(ss || '0', 10),
    0,
  );
  return Number.isNaN(d.getTime()) ? null : d;
}

function parseHistoryMutation(rawDataJson: string): ParsedHistoryMutation | null {
  ...
  if (!description || !classifyMaderaHistoryDescription(description)) return null;
  const amount =
    parseHistoryAmount(raw.debet)
  ...
  return {
    ...
    description,
    rawDataJson,
    rawHash: createHash('sha256').update(rawDataJson).digest('hex'),
  };
}

async function fetchMaderaHistory(account: SettlementAccountContext): Promise<ParsedHistoryMutation[]> {
  ...
  return history.mutations
    ...
    .filter((item): item is ParsedHistoryMutation => item !== null)
    .sort((left, right) => right.transactionTime.getTime() - left.transactionTime.getTime());
}

async function persistMaderaHistoryMutations(
  accountId: string,
  entries: ParsedHistoryMutation[],
): Promise<void> {
  for (const entry of entries) {
    const existing = await db.mutation.findUnique({ where: { rawHash: entry.rawHash }, select: { id: true } });
...
function findSettlementTransferMatch(
  settlement: {
    amount: number;
  ...
  entries: ParsedHistoryMutation[],
): { transfer: ParsedHistoryMutation; fee: ParsedHistoryMutation | null } | null {
  ...
  const transfer = entries.find((entry) => {
    if (entry.type !== 'debit' || entry.amount !== settlement.amount) return false;
    ...
    if (!desc.includes('BI FAST OUT')) return false;
    if (ownerText && desc.includes(ownerText)) return true;
    if (bankText && desc.includes(bankText)) return true;
    return true;
  });
  if (!transfer) return null;
  const fee = entries.find((entry) => {
    if (entry.type !== 'debit') return false;
    if (!entry.description.toUpperCase().includes('BIAYA TRANSFER BI FAST')) return false;
...
async function resolveSettlementAccountContext(qrisAccountId: string): Promise<{
  account: SettlementAccountContext;
...
function shouldFallbackToAppGateway(error: unknown): boolean {
  if (!(error instanceof Error)) return false;
  return error.message.toLowerCase().includes('web session cookie');
}

async function listSettlementTransferBanksInternal(
  account: SettlementAccountContext,
  fallbackIndex: number,
): Promise<Array<{ code: string; fee: number; name: string; status: string }>> {
  try {
    const panel = await fetchOrkutTransferBanks(account, fallbackIndex);
    ...
    if (!shouldFallbackToAppGateway(error)) throw error;
  }

  const overview = await appGateway.fetchMaderaTransferOverview(account);
  if (!overview?.banks) return [];
...
async function inquireSettlementBankPreferred(
  account: SettlementAccountContext,
  fallbackIndex: number,
  input: {
    amount?: number;
  ...
  if (input.fromWallet === 'madera') {
    try {
      const inquiry = await inquireOrkutBankAccount(account, fallbackIndex, {
        bankCode: input.bankCode,
        accountNumber: input.bankAccount,
        amount: Math.max(input.amount ?? 0, 10_000),
      });
      ...
      return {
        ...
        raw: JSON.parse(inquiry.rawJson || '{}') as Record<string, unknown>,
  ...
  const inquiry = await appGateway.inquireBankAccount(account, {
    sourceWallet: input.fromWallet,
    bankCode: input.bankCode,
    accountNumber: input.bankAccount,
    amount: input.fromWallet === 'madera'
      ? Math.max(input.amount ?? 0, 10_000)
      : input.amount,
  });
...
async function transferSettlementBankPreferred(
  account: SettlementAccountContext,
  fallbackIndex: number,
  params: {
    accountName: string;
  ...
  try {
    const transfer = await transferOrkutBankFromMadera(account, fallbackIndex, {
    ...
    const raw: Record<string, unknown> = JSON.parse(transfer.rawJson || '{}') as Record<string, unknown>;
    let status = transfer.status;
    let success = transfer.success;
    let message = transfer.message ?? null;

    if (transfer.success && transfer.redirectUrl && params.pin) {
      const pinResult = await appGateway.finalizeMaderaTransferPin(transfer.redirectUrl, params.pin, account);
  ...
  const transfer = await appGateway.transferBankFromMadera(account, {
    bankCode: params.bankCode,
    bankName: params.bankName,
    accountNumber: params.accountNumber,
    accountName: params.accountName,
...
async function persistAppSettlementBalances(
  accountId: string,
  params: {
  ...
  if (params.mainBalance !== undefined) updateData.lastMainBalance = params.mainBalance;
  if (params.qrisBalance !== undefined) updateData.lastQrisBalance = params.qrisBalance;
  if (params.maderaBalance !== undefined) updateData.lastMaderaBalance = params.maderaBalance;
  if (params.raw) updateData.lastBalanceSyncRawJson = JSON.stringify(params.raw);
...
export async function createSettlement(
  ...
  const settlement = await db.settlementRequest.create({
    data: {
  ...
  logger.info({ settlementId: settlement.id, ...validated }, 'Settlement request created');
 ...
 */
export async function processSettlement(settlementId: string): Promise<ProcessSettlementResult | void> {
  const settlement = await db.settlementRequest.findUnique({
    where: { id: settlementId },
  });

  if (!settlement) {
    logger.warn({ settlementId }, 'processSettlement: not found');
    return;
  }

  if (settlement.status !== 'pending') {
    logger.debug({ settlementId, status: settlement.status }, 'processSettlement: not pending, skipping');
    return;
  }
  ...
  try {
    if (settlement.fromWallet === 'qris' && settlement.toWallet === 'utama') {
      ...
      const beforeTerms = await appGateway.fetchQrisWithdrawTerms(account);
      ...
      if (availableQris < settlement.amount) {
        throw new Error(
          `Saldo QRIS tidak cukup (saldo: ${availableQris}, diminta: ${settlement.amount})`,
        );
      }

      const remoteResult = await appGateway.withdrawQris(account, settlement.amount);
      ...
      const [afterTerms, mainBalanceHistory] = await Promise.all([
        ...
        appGateway.fetchBalanceHistory(account),
      ]);
      ...
      logger.info(
        { settlementId, accountCode: account.code, amount: settlement.amount },
        'Settlement QRIS -> utama processed via app OrderKuota',
      );
      return {
        settlementId,
        status: 'done',
    ...
    if (settlement.fromWallet === 'utama' && settlement.toWallet === 'madera') {
      ...
      const beforeMainHistory = await appGateway.fetchBalanceHistory(account);
      ...
      if (availableMain < settlement.amount) {
        throw new Error(
          `Saldo utama tidak cukup (saldo: ${availableMain}, diminta: ${settlement.amount})`,
        );
      }

      const remoteResult = await appGateway.topupMadera(account, settlement.amount);
      if (!remoteResult.success) {
        throw new Error(remoteResult.message || 'Topup Madera gagal diproses.');
      }

      const afterMainHistory = await appGateway.fetchBalanceHistory(account);
      const maderaSnapshot = await syncOrkutBalanceSnapshot(account, fallbackIndex).catch(() => null);
      ...
      logger.info(
        { settlementId, accountCode: account.code, amount: settlement.amount, referenceNo: remoteResult.detailsId },
        'Settlement utama -> madera processed via app OrderKuota',
      );
      return {
        settlementId,
        status: 'done',
    ...
    if (settlement.toWallet === 'bank') {
      ...
      try {
        const _mov = await appGateway.fetchMaderaTransferOverview(account);
        if (_mov && typeof _mov.accountBalance === 'number') {
          ...
          if (_mov.accountBalance !== account.lastMaderaBalance) {
            await db.qrisAccount.update({ where: { id: account.id }, data: { lastMaderaBalance: _mov.accountBalance } }).catch(() => {});
          }
        }
    ...
    if (settlement.fromWallet === 'utama' || settlement.fromWallet === 'madera') {
      const balance = await getWalletBalance(settlement.fromWallet);
      if (balance < settlement.amount) {
        throw new Error(
          `Saldo ${settlement.fromWallet} tidak cukup (saldo: ${balance}, diminta: ${settlement.amount})`,
        );
      }
    }
    ...
    if (settlement.fromWallet === 'qris') {
      const utamaBalance = await getWalletBalance('utama');
      if (utamaBalance < settlement.amount) {
        throw new Error(
          `Saldo utama tidak cukup untuk settlement qris (saldo: ${utamaBalance}, diminta: ${settlement.amount})`,
        );
      }
      ...
      // Debit source wallet
      await recordWalletEntry({
        walletCode: settlement.fromWallet as 'utama' | 'madera',
        amount: -settlement.amount,
        refType: 'settlement_out',
        refId: settlementId,
        description: `Settlement ke ${settlement.toWallet} — #${settlementId.slice(0, 8)}`,
    ...
    logger.info({ settlementId, referenceNo, amount: settlement.amount }, 'Settlement processed');
    return {
      settlementId,
      status: 'done',
      referenceNo,
    ...
    logger.error({ settlementId, error: errorMsg }, 'Settlement processing failed');
    return {
      settlementId,
      status: 'failed',
      message: errorMsg,
...
export async function inquireSettlementBankAccount(input: {
  amount?: number;
  bankAccount: string;
  bankCode: string;
  fromWallet: 'utama' | 'madera';
...
export async function listSettlementTransferBanks(qrisAccountId: string): Promise<Array<{
  code: string;
  fee: number;
  name: string;
  status: string;
...
export async function reconcileProcessingMaderaTransfers(accountId?: string): Promise<number> {
  const where: Record<string, unknown> = {
    fromWallet: 'madera',
    toWallet: 'bank',
    status: 'processing',
  };
  if (accountId) where.qrisAccountId = accountId;

  const pendingSettlements = await db.settlementRequest.findMany({
    where,
    orderBy: { createdAt: 'asc' },
  });
  if (pendingSettlements.length === 0) return 0;

  const byAccount = new Map<string, typeof pendingSettlements>();
  for (const settlement of pendingSettlements) {
    if (!settlement.qrisAccountId) continue;
  ...
  for (const [qrisAccountId, settlements] of byAccount.entries()) {
    ...
    const historyEntries = await fetchMaderaHistory(account).catch((err) => {
      logger.warn({ err, accountCode: account.code }, 'reconcileProcessingMaderaTransfers: unable to fetch madera history');
      return [] as ParsedHistoryMutation[];
    });
  ...
  if (reconciledCount > 0) {
    logger.info({ reconciledCount, accountId }, 'Processing Madera transfers reconciled from history');
  }
...
export async function listSettlements(opts?: {
  ...
  const [settlements, total] = await Promise.all([
    ...
    db.settlementRequest.count({ where }),
  ]);
