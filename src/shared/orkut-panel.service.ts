...
type OrkutHeaders = Record<string, string>;
...
function parseObjectBalance(data: unknown, keys: string[]): number | undefined {
...
function buildHeaders(cookies: string, referer: string, userAgent?: string): OrkutHeaders {
  return {
    Cookie: cookies,
    'User-Agent':
...
function looksLikeWebCookie(value: string): boolean {
...
function readTransferError(payload: Record<string, unknown> | null, fallbackStatus?: number): string {
  if (!payload) {
...
function parseTransferBankOptions(payload: Record<string, unknown> | null): OrkutTransferBankOption[] {
  if (!payload) return [];
  ...
  const mappedFromRecord = nestedBanksRecord
    ? Object.entries(nestedBanksRecord).map(([code, raw]) => {
        ...
        } satisfies OrkutTransferBankOption;
      })
    : [];
  ...
  const deduped = new Map<string, OrkutTransferBankOption>();
...
async function postOrkutBankTransfer(
  account: Pick<QrisAccount, 'cookiesEncrypted' | 'webCookiesEncrypted' | 'webUserAgent' | 'orkutAccountIndex'>,
  ...
  const balanceBaseUrl = config.ORKUT_BALANCE_BASE_URL || ORKUT_MUTATION_BASE_URL;
...
function resolveEncryptedCookieSource(
  account: Pick<QrisAccount, 'cookiesEncrypted' | 'webCookiesEncrypted'>,
): string | null {
  return account.webCookiesEncrypted ?? account.cookiesEncrypted ?? null;
}

function resolveWebPanelCookie(
  account: Pick<QrisAccount, 'cookiesEncrypted' | 'webCookiesEncrypted'>,
): string | null {
  const encryptedCookieSource = resolveEncryptedCookieSource(account);
  if (!encryptedCookieSource) return null;

  try {
    const decrypted = decrypt(encryptedCookieSource);
...
function buildMerchantUrl(baseUrl: string, pathname: string): string {
  const normalizedBase = baseUrl.endsWith('/') ? baseUrl : `${baseUrl}/`;
  return new URL(pathname.replace(/^\/+/, ''), normalizedBase).toString();
}

async function fetchJson(url: string, headers: OrkutHeaders): Promise<unknown | null> {
...
export function classifyOrkutMutationDescription(description: string): OrkutMutationCategory {
...
export function summarizeOrkutAccountBalances(
  accounts: Array<{
...
export function resolveOrkutAccountIndex(
  account: Pick<QrisAccount, 'orkutAccountIndex'>,
  fallbackIndex: number,
): number {
...
export async function syncOrkutBalanceSnapshot(
  account: Pick<
    QrisAccount,
    | 'cookiesEncrypted'
    | 'webCookiesEncrypted'
  ...
  >,
  fallbackIndex: number,
): Promise<OrkutBalanceSnapshot | null> {
  const cookies = resolveWebPanelCookie(account);
  ...
  const combinedWithdrawEnabled = combined
    ? parseBooleanValue((combined as Record<string, unknown>).withdraw_enabled)
    : undefined;
  const combinedWithdrawMin = combined
    ? parseAmountValue((combined as Record<string, unknown>).withdraw_min)
    : undefined;
  const combinedWithdrawMax = combined
    ? parseAmountValue((combined as Record<string, unknown>).withdraw_max)
    : undefined;

  let mainBalance = combinedMain;
  let qrisBalance = combinedQris;
  let maderaBalance = combinedMadera;
  let payloadSource: 'combined' | 'merged' = 'combined';
  let rawPayload: unknown = combined;
  ...
  if (mainBalance === undefined || qrisBalance === undefined || maderaBalance === undefined) {
    ...
    mainBalance ??= parseObjectBalance(mainPayload, ['total_balance', 'balance', 'main_balance']);
    qrisBalance ??= parseObjectBalance(qrisPayload, ['total_balance', 'balance', 'qris_balance', 'pending_balance']);
    maderaBalance ??= parseObjectBalance(maderaPayload, ['total_balance', 'balance', 'madera_balance']);
...
export async function performOrkutSettlementAction(
  account: Pick<
    QrisAccount,
    'code' | 'cookiesEncrypted' | 'webCookiesEncrypted' | 'webUserAgent' | 'orkutAccountIndex'
  >,
  fallbackIndex: number,
  action: OrkutSettlementAction,
  amount: number,
): Promise<OrkutSettlementActionResult> {
  const cookies = resolveWebPanelCookie(account);
  if (!cookies) {
    throw new Error('Merchant belum punya Web Session Cookie yang valid untuk settlement panel.');
  }
  ...
  const referenceNo =
    (typeof payload.reference_no === 'string' && payload.reference_no) ||
...
export async function fetchOrkutTransferBanks(
  account: Pick<QrisAccount, 'cookiesEncrypted' | 'webCookiesEncrypted' | 'webUserAgent' | 'orkutAccountIndex'>,
  fallbackIndex: number,
): Promise<{
  accountIndex: number;
...
export async function inquireOrkutBankAccount(
  account: Pick<QrisAccount, 'cookiesEncrypted' | 'webCookiesEncrypted' | 'webUserAgent' | 'orkutAccountIndex'>,
  fallbackIndex: number,
  params: {
...
export async function transferOrkutBankFromMadera(
  account: Pick<QrisAccount, 'cookiesEncrypted' | 'webCookiesEncrypted' | 'webUserAgent' | 'orkutAccountIndex'>,
  fallbackIndex: number,
  params: {
    accountName: string;
  ...
  const results = asRecord(payload?.results);
  const redirectUrl =
    readStringValue(payload?.redirect_url)
    || readStringValue(results?.redirect_url)
    || null;
  const referenceNo =
    readStringValue(payload?.reference_no)
