...
function encodeField(field: TlvField): string {
...
function upsertPointOfInitiationMethod(fields: TlvField[]): TlvField[] {
  const next = [...fields];
...
function upsertTransactionAmount(fields: TlvField[], amount: number): TlvField[] {
  const next = fields.filter((field) => field.tag !== '54' && field.tag !== '63');
  const amountField: TlvField = { tag: '54', value: String(amount) };
